...
export const getAdminChats = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.user || req.user.role !== 'ADMIN') throw new AppError('Unauthorized', 401);
    
    // Fetch all unique chat documents
    const chatsSnap = await db.collection('support_chats').orderBy('updatedAt', 'desc').get();
...
export const getAdminMessages = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.user || req.user.role !== 'ADMIN') throw new AppError('Unauthorized', 401);
...
export const adminSendMessage = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.user || req.user.role !== 'ADMIN') throw new AppError('Unauthorized', 401);
    const userId = req.params.userId as string;
    const { text } = req.body;
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        if (userData?.email) {
          await sendSupportReplyEmail(userData.email, userData.name || 'Valued User', text);
        }
      }
    } catch (emailErr) {
