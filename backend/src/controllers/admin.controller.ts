...
export const rejectSeller = async (
	...
	try {
		...
		const _sellerDoc = await db.collection("sellers").doc(id).get();
		const userDoc = await db.collection("users").doc(id).get();
			...
			productsSnapshot.docs.map(async (doc) => {
				...
				if (data.sellerId) {
					try {
						...
						console.warn(
							`[Admin Vault] Missing seller link for product ${doc.id}`,
						);
...
export const listOrders = async (
	...
	try {
		...
		// 🚀 BATCH FETCHING
		const productRefs = ordersData
			.map((o: any) =>
				o.productId ? db.collection("products").doc(o.productId) : null,
			)
			.filter(Boolean) as admin.firestore.DocumentReference[];
		const buyerRefs = ordersData
			.map((o: any) =>
				o.buyerId ? db.collection("users").doc(o.buyerId) : null,
			)
			.filter(Boolean) as admin.firestore.DocumentReference[];
		const sellerRefs = ordersData
			.map((o: any) =>
				o.sellerId ? db.collection("sellers").doc(o.sellerId) : null,
			)
			.filter(Boolean) as admin.firestore.DocumentReference[];
		...
		const buyerDocs = allDocs.slice(
			productRefs.length,
			productRefs.length + buyerRefs.length,
		);
		...
		const sellerUserRefs = sellerDocs
			.map((doc: any) =>
				doc.exists ? db.collection("users").doc(doc.id) : null,
			)
			.filter(Boolean) as admin.firestore.DocumentReference[];
		...
		const productMap = new Map(
			productDocs.map((d) => [d.id, d.exists ? d.data() : null]),
		);
		const buyerMap = new Map(
			buyerDocs.map((d) => [d.id, d.exists ? d.data() : null]),
		);
		const sellerMap = new Map(
			sellerDocs.map((d) => [d.id, d.exists ? d.data() : null]),
		);
		const sellerUserMap = new Map(
			sellerUserDocs.map((d) => [d.id, d.exists ? d.data() : null]),
		);
		...
		const orders = ordersData.map((o: any) => {
			...
			return {
				...
				seller: {
					...(sData || {}),
					user: suData || { name: "Verified Merchant" },
				},
			};
		});
...
export const updateSettings = async (
	...
	try {
		...
		if (sellerAutoVerify !== undefined)
			updateData.sellerAutoVerify = sellerAutoVerify;
		if (minPayoutAmount !== undefined)
			updateData.minPayoutAmount = minPayoutAmount;
...
export const getDashboardStats = async (
	...
	try {
		...
		const recentOrdersSnap = await db
			...
			.limit(5)
			.get();
		const ordersData = recentOrdersSnap.docs.map((doc: any) => ({
			id: doc.id,
			...doc.data(),
		}));
		...
		const recentOrders = ordersData.map((o: any) => {
			const pData = productMap.get(o.productId);
			const bData = buyerMap.get(o.buyerId);
			const _sData = sellerMap.get(o.sellerId);
		...
		const topSellersSnap = await db
			.collection("sellers")
			.orderBy("totalEarnings", "desc")
			.limit(5)
			.get();
		const topSellerUserRefs = topSellersSnap.docs.map((doc: any) =>
			db.collection("users").doc(doc.id),
		);
		const topSellerUserDocs =
			topSellerUserRefs.length > 0 ? await db.getAll(...topSellerUserRefs) : [];
