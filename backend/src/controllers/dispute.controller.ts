...
export const createDispute = async (
	...
	try {
		...
		if (!["SHIPPED", "DELIVERED"].includes(orderData.status)) {
			throw new AppError(
				"Disputes can only be opened for shipped or delivered items",
				400,
			);
		}
		...
		const uploadResults = files
			? await Promise.all(
					files.map((f) => uploadToCloudinary(f.buffer, "disputes")),
				)
			: [];
		...
		res.status(201).json({
			...
			dispute: { id: disputeRef.id, ...disputeData },
		});
	} catch (error) {
		next(error);
	}
	...
	try {
		...
		res.json({
			...
			order: orderDoc.exists
				? { id: orderDoc.id, ...orderDoc.data(), product: productInfo }
				: null,
			buyer: buyerDoc.exists
				? { name: buyerDoc.data()?.name }
				: { name: "Unknown" },
			seller: sellerDoc.exists
...
export const adminResolveDispute = async (
	...
	try {
		...
		if (resolution === "REFUND_BUYER") {
			...
			const platformFee = orderData.totalPrice * 0.2;
