...
export const getProductsBulk = async (
	req: Request,
	res: Response,
	next: NextFunction,
) => {
	try {
		...
		if (ids.length > 30)
			throw new AppError("Cannot fetch more than 30 products at once", 400);
...
export const createProduct = async (
	...
	try {
		...
		const imageUploadResults = await Promise.all(
			imageFiles.map((file) =>
				uploadToCloudinary(file.buffer, "products", false),
			),
		);
		const videoUploadResults = await Promise.all(
			videoFiles.map((file) =>
				uploadToCloudinary(file.buffer, "products", true),
			),
		);

		const imageUrls = imageUploadResults.map((r) => r.url);
		const videoUrl =
			videoUploadResults.length > 0 ? videoUploadResults[0].url : null;
		...
		const productData = {
			...
			category,
			description,
			stock: parseInt(stock as string, 10) || 1,
			originalPacking: originalPacking === "true" || originalPacking === true,
			invoiceAvailable:
				invoiceAvailable === "true" || invoiceAvailable === true,
			images: imageUrls,
			videoUrl: videoUrl,
			status: "PENDING",
			views: 0,
...
export const softDeleteProduct = async (
	...
	try {
		...
		if (!activeOrders.empty) {
			throw new AppError(
				"Cannot delete a product with active orders. Please fulfill or resolve the current transactions first.",
				400,
			);
		}
...
export const adminRejectProduct = async (
	req: Request,
	res: Response,
	next: NextFunction,
) => {
...
export const visualSearch = async (
	...
	try {
		...
		const { suggestedTags, detectedCategory } =
			await AIService.performVisualSearch(imageUrl);
