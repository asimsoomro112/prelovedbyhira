...
export const submitIdentity = async (
	...
	try {
		...
		const { extractedData, error } =
			await AIService.scanIdentityDocument(cnicFront);
		...
		const enteredName =
			fullNameEntered?.toLowerCase() || req.user?.name?.toLowerCase() || "";

		// Simple inclusion check or fuzzy match could be better, but we start with equality/inclusion
		const isMatch =
			extractedName.includes(enteredName) ||
			enteredName.includes(extractedName);
		...
		res.json({
			success: true,
			message:
				"Identity verified successfully by ReVault AI. Please proceed to upload your selfie.",
...
export const submitSelfie = async (
	...
	try {
		...
		await db.runTransaction(async (transaction) => {
			...
			if (payoutMethod && payoutDetails) {
				...
				try {
					...
					console.warn(
						"[PAYOUT_SAVE] Failed to parse payoutDetails, using as raw string",
					);
				...
				transaction.set(accountRef, {
					...
					details:
						...
						detailsObj?.iban ||
						(typeof payoutDetails === "string" ? payoutDetails : ""),
		...
		res.json({
			success: true,
			message:
				"Selfie uploaded and payout account saved. Waiting for admin approval.",
		});
	} catch (error) {
		next(error);
	}
...
export const getDashboardStats = async (
	...
	try {
		...
		const recentOrdersRaw = recentOrdersSnapshot.docs.map((doc) => ({
			id: doc.id,
			...doc.data(),
		}));
...
export const getPublicSellerProfile = async (
	...
	try {
		...
		res.json({
			...
			bio:
				sellerData.bio || userData.bio || "Premium seller on Preloved Market.",
