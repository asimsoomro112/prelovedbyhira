...
export const createOrder = async (
	...
	try {
		...
		const result = await db.runTransaction(async (transaction) => {
			...
			const platformFee = sellingPriceTotal * 0.2; // 20% Admin Commission
			const netAmount = sellingPriceTotal * 0.8; // 80% to Seller
		...
		res.status(201).json({
			order: result,
			message:
				"Order created and stock reserved. Please upload payment receipt to finalize.",
		});
	} catch (error) {
		next(error);
	}
	...
	try {
		...
		const results = await db.runTransaction(async (transaction) => {
			...
			for (let i = 0; i < items.length; i++) {
				...
				if ((product.stock || 0) < qty) {
					throw new AppError(
						`"${product.title}" — only ${product.stock || 0} left in stock`,
						400,
					);
				}
				if (product.sellerId === buyerId) {
			...
			for (let i = 0; i < items.length; i++) {
				...
				const unitPrice = Number(product.sellingPrice);
				const sellingPriceTotal = unitPrice * qty;
				const platformFee = sellingPriceTotal * 0.2;
				const netAmount = sellingPriceTotal * 0.8;
		...
		for (const order of results) {
			try {
				...
				console.warn(
					`[Vault] Email send failed for order ${order.id}:`,
					emailErr,
				);
			}
		}
	...
	try {
		...
		if (buyerDoc.exists) {
			...
			await EmailService.sendOrderStatusUpdate(
				...
				id,
				"SHIPPED",
				productDoc.data()?.title || "Luxury Item",
			);
		}
...
export const confirmDelivery = async (
	...
	try {
		...
		res.json({
			message: "Delivery confirmed, funds released to seller balance",
		});
	} catch (error) {
		next(error);
	}
...
export const getMyOrders = async (
	...
	try {
		...
		const productRefs = ordersData.map((o: any) =>
			db.collection("products").doc(o.productId),
		);
		const sellerRefs = ordersData.map((o: any) =>
			db.collection("sellers").doc(o.sellerId),
		);
		...
		const sellerUserRefs = sellerDocs
			.map((doc) => (doc.exists ? db.collection("users").doc(doc.id) : null))
			.filter(Boolean) as admin.firestore.DocumentReference[];
		const sellerUserDocs =
			sellerUserRefs.length > 0 ? await db.getAll(...sellerUserRefs) : [];
		...
		const orders = ordersData.map((o: any, idx: number) => {
			...
			return {
				...o,
				product: pDoc?.exists
					? { id: pDoc.id, ...pDoc.data() }
					: { title: "Luxury Item", images: [], brand: "Premium" },
...
export const getSellerOrders = async (
	...
	try {
		...
		const buyerRefs = ordersData.map((o: any) =>
			db.collection("users").doc(o.buyerId),
		);
...
export const submitPaymentProof = async (
	...
	try {
		...
		try {
			aiResults = await AIService.verifyPaymentReceipt(
				proofUrl,
				order.totalPrice,
			);
		} catch (_err) {
			console.warn(
				"[ReVault AI] Payment audit failed, queuing for manual review.",
			);
		}
...
export const adminConfirmPayment = async (
	...
	try {
		...
		await db.runTransaction(async (transaction) => {
			...
			const _product = productDoc.data()!;
			transaction.update(orderRef, {
				status: "PAID",
				adminConfirmed: true,
				updatedAt: new Date().toISOString(),
		...
		if (sellerId) {
			await NotificationService.create({
				...
				message:
					"Payment received confirmed. Now send product for shipping to the customer provided address.",
				type: "ORDER_UPDATE",
			});
...
export const submitReview = async (
	...
	try {
		...
		await db.runTransaction(async (transaction) => {
			...
			const newRating =
				(currentRating * reviewCount + Number(rating)) / newReviewCount;
			...
			if (sellerDoc.exists) {
				...
				const sNewRating =
					(sRating * sReviewCount + Number(rating)) / sNewCount;
		...
		res.json({
			message: "Review submitted! Your feedback helps the luxury community.",
		});
	} catch (error) {
		next(error);
	}
