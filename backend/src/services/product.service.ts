...
export class ProductService {
	static async getProducts(filters: {
		...
		console.log(
			`[ProductService] Fetching products. Sort: ${sortBy}, Filters: ${JSON.stringify({ category, condition, size })}`,
		);
		...
		const buildQuery = (level: "complex" | "simple" | "ultra-safe") => {
			...
			const hasPriceRange =
				...
				(maxPrice !== undefined && maxPrice < 1000000);
			...
			if (hasPriceRange) {
				if (minPrice && minPrice > 0)
					q = q.where("sellingPrice", ">=", minPrice);
				if (maxPrice && maxPrice < 1000000)
					q = q.where("sellingPrice", "<=", maxPrice);
			}
			return q;
		};
		...
		const mapProductsWithSellers = async (
			docs: admin.firestore.QueryDocumentSnapshot[],
		) => {
			return Promise.all(
				docs.map(async (doc) => {
					const data = doc.data();
					let seller: { name: string; avatar?: string } = {
						name: "ReVault Member",
					};
		...
		try {
			...
			try {
				 ...
				 */
				console.warn(
					"[ReVault AI] Complex index missing. Using Global In-Memory Sort for accuracy.",
				);
				...
				const allProducts = await mapProductsWithSellers(allDocsSnapshot.docs);

				// Global Sort
				if (sortBy === "price_asc")
					allProducts.sort((a: any, b: any) => a.sellingPrice - b.sellingPrice);
				else if (sortBy === "price_desc")
					allProducts.sort((a: any, b: any) => b.sellingPrice - a.sellingPrice);
				else if (sortBy === "popular")
					allProducts.sort((a: any, b: any) => (b.views || 0) - (a.views || 0));
				else
					allProducts.sort(
						(a: any, b: any) =>
				...
				console.error(
					"[ReVault AI] Critical Query Failure, using ultra-safe mode.",
				);
		...
		try {
			// Safe Count
			const countQuery: admin.firestore.Query = db
				.collection("products")
				.where("status", "==", "ACTIVE");
