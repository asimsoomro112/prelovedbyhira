...
function Counter({ value, c }: { value: string; c: typeof T.dark }) {
  ...
  return (
    <motion.span ref={ref} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }} style={{ color: c.gold, fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic' }} className="text-4xl md:text-5xl font-bold">
      {value}
    </motion.span>
  );
}
...
function TiltCard({ children, className, style }: { children: React.ReactNode; className?: string; style?: React.CSSProperties }) {
  ...
  return (
    <motion.div onMouseMove={onMove} onMouseLeave={onLeave} style={{ rotateX, rotateY, transformPerspective: 1200, ...style }} whileHover={{ scale: 1.03, z: 20 }} className={className}>
      {children}
    </motion.div>
  );
}

function ProtocolCard({ icon: Icon, title, desc, badge, color, c, i }: { icon: any; title: string; desc: string; badge: string; color: string; c: typeof T.dark; i: number }) {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: "-80px" });
  const accentMap: Record<string, string> = { green: c.green, gold: c.gold, blue: '#60A5FA', red: c.red };
  const accent = accentMap[color] ?? c.gold;
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.1, duration: 0.7, ease: [0.16, 1, 0.3, 1] }} className="h-full">
      <TiltCard style={{ background: c.surface, borderRadius: 32, border: `1px solid ${c.border}`, padding: '2.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', cursor: 'default', height: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <span style={{ padding: '5px 14px', borderRadius: 100, fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', background: `${accent}14`, border: `1px solid ${accent}28`, color: accent }}>{badge}</span>
        </div>
        <div>
    ...
    <motion.div ref={ref} initial={{ opacity: 0, scale: 0.94 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
      <motion.div onMouseMove={onMove} onMouseLeave={onLeave} style={{ rotateX, rotateY, transformPerspective: 1000 }} className={`group relative rounded-[40px] overflow-hidden cursor-pointer ${highlight ? 'h-[560px]' : 'aspect-[3/4]'}`} onClick={() => router.push(`/product/${item.id}`)} whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }}>
        ...
        <div className="absolute top-5 left-5 z-10 flex flex-col gap-2">
          <span style={{ padding: '5px 14px', borderRadius: 100, fontSize: 9, fontWeight: 800, letterSpacing: '0.2em', textTransform: 'uppercase', background: item.condition === 'NEW' ? '#10B981' : 'rgba(0,0,0,0.5)', backdropFilter: 'blur(8px)', border: item.condition === 'NEW' ? '1px solid #34D399' : `1px solid ${c.border}`, color: item.condition === 'NEW' ? '#fff' : c.gold, boxShadow: item.condition === 'NEW' ? '0 0 15px rgba(16, 185, 129, 0.4)' : 'none' }}>{item.condition}</span>
          {highlight && <span style={{ padding: '5px 14px', borderRadius: 100, fontSize: 9, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', background: 'rgba(239,68,68,0.7)', color: '#fff', display: 'flex', alignItems: 'center', gap: 5 }}><TrendingUp size={10} /> Most Wanted</span>}
        </div>
        <div className="absolute bottom-0 inset-x-0 p-7 z-10 space-y-3">
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
            <div className="flex-1">
              ...
              {item.originalPrice > item.sellingPrice && <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)', textDecoration: 'line-through', marginTop: 2 }}>Rs. {item.originalPrice?.toLocaleString()}</p>}
            </div>
            <motion.div whileHover={{ scale: 1.15, rotate: -12 }} className="w-11 h-11 rounded-2xl flex items-center justify-center" style={{ background: c.gold, color: '#000' }}><ArrowRight size={18} /></motion.div>
          </div>
        </div>
      </motion.div>
    </motion.div>
...
export default function HomeClient({ trending, arrivals, productCount }: { trending: any[], arrivals: any[], productCount: number }) {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ScrollTrigger.create({ trigger: sectionRef.current, start: "top top", end: "+=350%", pin: true, scrub: 1, onUpdate: (self) => { scrollYProgress.set(self.progress); } });
    });
    return () => ctx.revert();
  }, [scrollYProgress]);
  ...
  return (
    <div style={{ background: c.bg, color: c.text, overflowX: 'hidden', transition: 'background 0.5s, color 0.5s' }} className="min-h-screen selection:bg-amber-400/30">
      ...
      <section className="w-full min-h-[85vh] flex flex-col items-center justify-center relative px-6 text-center" style={{ background: c.bg }}>
        <div style={{ position: 'absolute', inset: 0, background: theme === 'dark' ? 'radial-gradient(ellipse 80% 60% at 50% 0%, #1a0a2e 0%, #06060A 65%)' : 'radial-gradient(ellipse 80% 60% at 50% 0%, #ede8ff 0%, #FAF8F4 65%)' }} />
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }} className="relative z-10 max-w-5xl space-y-8 md:space-y-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-ultra" style={{ border: `1px solid ${c.gold}40`, background: `${c.gold}12` }}>
            <motion.div animate={{ scale: [1, 1.3, 1], opacity: [1, 0.5, 1] }} transition={{ repeat: Infinity, duration: 2 }} style={{ width: 6, height: 6, borderRadius: '50%', background: c.gold }} />
            <span className="text-[9px] font-black uppercase tracking-[0.4em] text-gold-500">Pakistan's #1 Luxury Resale</span>
          </div>
          
          <h1 className="text-[clamp(3.5rem,10vw,9rem)] leading-[0.85] font-bold tracking-tighter" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
            Wear Art.<br />
            <em style={{ backgroundImage: theme === 'dark' ? 'linear-gradient(90deg, #C9A227, #F0D060, #C9A227)' : 'linear-gradient(90deg, #7C3AED, #DB2777, #A37118)', WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent', display: 'inline-block' }}>Own Legacy.</em>
          </h1>
          ...
          <div className="flex flex-wrap items-center justify-center gap-5 md:gap-8 pt-4">
            <Link href="/products" className="group relative px-12 py-6 bg-gold-500 text-white rounded-2xl font-bold overflow-hidden transition-all hover:scale-105 active:scale-95 flex items-center gap-3 shadow-2xl shadow-gold-500/20">
              Shop Collection <ArrowRight size={20} />
              <motion.div animate={{ x: ['-100%', '200%'] }} transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut', repeatDelay: 1 }} style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.3), transparent)', transform: 'skewX(-20deg)' }} />
            </Link>
            <Link href="/register" className="px-12 py-6 rounded-2xl border border-white/10 font-bold hover:bg-white/5 transition-all flex items-center gap-3 backdrop-blur-md">Start Selling <Zap size={18} /></Link>
          </div>
        </motion.div>
      </section>
      ...
      <section ref={sectionRef} className="w-full h-screen overflow-hidden relative" style={{ background: c.bg }}>
        <motion.div style={{ position: 'absolute', top: '20%', left: '50%', translateX: '-50%', width: 600, height: 600, borderRadius: '50%', background: theme === 'dark' ? 'radial-gradient(circle, rgba(139,92,246,0.2) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(124,58,237,0.15) 0%, transparent 70%)', filter: 'blur(80px)', scale: glowScale, opacity: glowAlpha, pointerEvents: 'none' }} />
        
        <div className="w-full h-full flex flex-col items-center justify-center relative">
          <div className="relative w-full h-full flex items-center justify-center pointer-events-none">
            <motion.svg viewBox="0 0 400 700" className="w-auto h-[85vh] max-h-[700px] overflow-visible">
              <defs>
              ...
              <motion.g style={{ opacity: threadAlpha }}>
                {THREADS.map(t => (
                  <motion.line key={t.id} x1={t.x} y1={t.y} x2={Number((t.x + Math.cos(t.ang * Math.PI/180)*t.len).toFixed(2))} y2={Number((t.y + Math.sin(t.ang * Math.PI/180)*t.len).toFixed(2))} stroke="url(#threadGrad)" strokeWidth="1.4" strokeLinecap="round" animate={{ opacity: [0, 0.7, 0.4], x: [0, Math.sin(t.id)*6, 0], y: [0, Math.cos(t.id)*6, 0] }} transition={{ delay: t.del, duration: 2.8, repeat: Infinity, repeatType: 'reverse' }} />
                ))}
              </motion.g>
      ...
      <section className="px-6 lg:px-8 py-20 overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-8 order-2 lg:order-1 text-center lg:text-left">
              <div className="space-y-4"><p className="text-[10px] md:text-[12px] font-black uppercase tracking-[0.6em] text-gold-400">Authentic • Curated • Trusted • Timeless</p><h3 className="text-4xl md:text-6xl font-display font-bold text-dark-900 dark:text-white leading-[1.1]">Welcome to the <br /><span className="italic text-gold-400">Digital Vault</span> of Luxury.</h3></div>
              <p className="text-dark-600 dark:text-gray-400 text-base md:text-lg max-w-xl mx-auto lg:mx-0 leading-relaxed">Every masterpiece in our collection is AI-verified and physically authenticated, ensuring your investment remains timeless. Step into a world where quality is never compromised.</p>
              <div className="pt-4 flex flex-wrap gap-4 justify-center lg:justify-start"><button className="px-8 py-4 bg-gold-400 text-black font-bold rounded-2xl hover:bg-gold-500 transition-all active:scale-95 shadow-lg shadow-gold-400/20">Enter The Vault</button></div>
            </motion.div>
            <motion.div initial={{ opacity: 0, scale: 0.9, x: 30 }} whileInView={{ opacity: 1, scale: 1, x: 0 }} viewport={{ once: true }} className="relative order-1 lg:order-2">
              <div className="relative aspect-square w-full max-w-[500px] mx-auto group">
                ...
                <div className="relative z-10 w-full h-full rounded-[48px] overflow-hidden shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] dark:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] bg-black p-0 border-none">
                  <Image src="/hero-vault.png" alt="The ReVault Door" width={1000} height={1000} priority={true} className="w-full h-full object-contain transition-transform duration-[10s] group-hover:scale-105" />
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <section style={{ background: c.surface, borderTop: `1px solid ${c.border}`, borderBottom: `1px solid ${c.border}`, padding: '2.5rem 1.5rem' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '2rem' }}>
          {stats.map(({ value, label, icon: Icon }, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1, duration: 0.6 }} viewport={{ once: true }} className="flex flex-col items-center gap-6 text-center">
      ...
      <section style={{ padding: '8rem 1.5rem' }}>
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-20">
            ...
            <h2 className="text-[clamp(2.5rem,5vw,5rem)] font-bold tracking-tight leading-[1.05]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>The <span className="italic text-gold-500">Preloved</span> Protocol.</h2>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-20">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }}>
              <div className="flex items-center gap-2 mb-3"><motion.div animate={{ scale: [1, 1.3, 1], opacity: [1, 0.5, 1] }} transition={{ repeat: Infinity, duration: 1 }} className="w-2 h-2 rounded-full bg-red-500" /><span className="text-[9px] font-black uppercase tracking-[0.4em] text-red-500">Live Market Hits</span></div>
              <h2 className="text-[clamp(2.5rem,5vw,5rem)] font-bold tracking-tight leading-[1.05]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Most <span className="italic text-gold-500">Wanted.</span></h2>
            </motion.div>
            <Link href="/products" className="px-7 py-3 border border-white/10 rounded-xl text-[11px] font-bold uppercase tracking-widest hover:bg-white/5 transition-all flex items-center gap-2">Explore Vault <ArrowRight size={14} /></Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {trending.map((item: any) => <ProductCard key={item.id} item={item} highlight c={c} />)}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '8rem 1.5rem' }}>
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }} className="mb-16">
            <h2 className="text-[clamp(2rem,4.5vw,4.5rem)] font-bold tracking-tight" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Fresh <span className="italic text-gold-500">Vaults.</span></h2>
            <div className="mt-4 h-1 w-16 bg-gradient-to-r from-gold-500 to-transparent rounded-full" />
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {arrivals.map((item: any) => <ProductCard key={item.id} item={item} c={c} />)}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '8rem 1.5rem', background: c.surfaceAlt }}>
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-[clamp(2rem,4vw,4rem)] font-bold mb-16" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Explore by <span className="italic text-gold-500">Category.</span></h2>
              ...
              <motion.div key={cat.slug} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }} viewport={{ once: true }}>
                <Link href={`/products?category=${cat.slug}`}>
                  <motion.div whileHover={{ y: -6, borderColor: `${c.gold}60`, background: `${c.gold}08` }} className="h-52 rounded-[32px] border border-white/5 bg-surface flex flex-col items-center justify-center gap-5 cursor-pointer transition-all">
                    ...
                    <div className="text-center"><p className="text-[11px] font-black uppercase tracking-widest">{cat.name}</p></div>
                  </motion.div>
                </Link>
              </motion.div>
            ))}
      ...
      <section style={{ padding: '8rem 1.5rem', background: c.surface }}>
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-20">
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="flex-1">
            ...
            <h2 className="text-[clamp(2.5rem,5vw,5rem)] font-bold tracking-tight leading-[1.05] mb-8" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Merchant <span className="italic text-gold-500">Intelligence.</span></h2>
            <p className="text-dark-400 text-lg font-light mb-8 leading-relaxed">Monetize your luxury wardrobe with Pakistan's most sophisticated resale engine. We handle the digital KYC, AI-assisted pricing, and insured logistics while you curate your legacy.</p>
            <div className="grid grid-cols-2 gap-6">
              {[
                ...
                <div key={i} className="space-y-2"><h4 className="font-bold text-gold-500 text-sm">{item.title}</h4><p className="text-xs text-dark-500 leading-relaxed">{item.desc}</p></div>
              ))}
            </div>
      ...
      <section style={{ padding: '8rem 1.5rem' }}>
        <div className="max-w-7xl mx-auto text-center">
          ...
          <h2 className="text-[clamp(2.5rem,5vw,5rem)] font-bold tracking-tight leading-[1.05] mb-20" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Voices of <span className="italic text-gold-500">Trust.</span></h2>
            ...
            {reviews.map((rev, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}>
                <TiltCard className="h-full bg-surface border border-white/5 rounded-[36px] p-10 relative text-left">
                  ...
                  <div className="flex gap-1 mb-6">{[...Array(5)].map((_, j) => <Star key={j} size={14} className="fill-gold-500 text-gold-500" />)}</div>
                  ...
                  <div className="flex items-center gap-4 pt-6 border-t border-white/5">
                    ...
                    <div><p className="font-bold text-sm">{rev.name}</p><p className="text-[10px] font-black uppercase tracking-widest text-gold-500">Verified Buyer of {rev.item}</p></div>
                  </div>
                </TiltCard>
              </motion.div>
            ))}
      ...
      <section style={{ padding: '8rem 1.5rem', background: c.surfaceAlt }}>
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h2 className="text-[clamp(2.5rem,6vw,6rem)] font-bold tracking-tighter leading-[0.95] mb-8" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Your next<br /><span className="italic text-gold-500">treasure awaits.</span></h2>
