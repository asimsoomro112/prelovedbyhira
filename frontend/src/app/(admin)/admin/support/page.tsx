	...
	return (
		<div className="max-w-7xl mx-auto space-y-8">
			...
			<div className="flex items-end justify-between">
				<div>
					<h1 className="text-4xl font-display font-bold text-dark-900 dark:text-cream-50">
						Support <span className="text-gold-400 italic">Inbox.</span>
					</h1>
					<p className="text-gray-500 mt-2">
						Manage all live customer and seller inquiries.
					</p>
				</div>
			</div>
			...
			<div className="glass-ultra crystal-border rounded-[32px] md:rounded-[40px] overflow-hidden flex h-[75vh] md:h-[70vh] shadow-xl relative">
				{/* Left: Chat List */}
				<div
					className={`w-full lg:w-1/3 border-r border-gold-400/10 bg-white/50 dark:bg-dark-900/50 flex flex-col ${showMobileChat ? "hidden lg:flex" : "flex"}`}
				>
								...
								>
									...
									<div className="flex-1 min-w-0">
										<div className="flex items-center justify-between">
											<h4 className="font-bold text-dark-900 dark:text-cream-50 truncate">
												{chat.userName}
											</h4>
										...
										<p className="text-[9px] text-gold-400 uppercase tracking-widest font-bold mt-2">
											{chat.userRole}
										</p>
									</div>
								</button>
				...
				<div
					className={`w-full lg:w-2/3 flex flex-col bg-cream-50/30 dark:bg-black/20 ${!showMobileChat ? "hidden lg:flex" : "flex"}`}
				>
					{activeChat ? (
						<>
							{/* Chat Header */}
							<div className="p-4 md:p-6 border-b border-gold-400/10 bg-white/50 dark:bg-dark-900/50 flex items-center gap-3 md:gap-4">
								...
								<div>
									<h3 className="font-bold text-lg text-dark-900 dark:text-cream-50">
										{activeChat.userName}
									</h3>
									<p className="text-xs text-gold-400 font-bold uppercase tracking-widest">
										{activeChat.userRole} • ID: {activeChat.userId.slice(-6)}
									</p>
								</div>
							</div>
							...
							>
								{messages.map((msg) => {
									const isAdmin =
										msg.senderId === user?.id || msg.senderRole === "ADMIN";
									return (
										<div
											key={msg.id}
										...
										>
											...
											>
												{msg.text}
												<p
													className={`text-[9px] mt-2 font-bold uppercase tracking-widest ${isAdmin ? "text-white/60" : "text-gray-400"}`}
												>
						...
						<div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-50 px-10">
							...
							<div>
								...
								<p className="text-sm mt-2">
									Select a user from the left pane to view their chat history
									and respond.
								</p>
							</div>
						</div>
