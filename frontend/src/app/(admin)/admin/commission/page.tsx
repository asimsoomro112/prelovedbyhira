...
import { PieChart, TrendingUp, Calendar, Search, Loader2, ArrowUpRight } from "lucide-react";
...
export default function CommissionTrackerPage() {
  ...
  const commissionOrders = orders?.filter((o: any) => 
    ["PAID", "SHIPPED", "DELIVERED", "CONFIRMED"].includes(o.status)
  ) || [];

  const filteredOrders = commissionOrders.filter((o: any) => 
    o.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
    o.seller?.user?.name?.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const totalCommission = commissionOrders.reduce((sum: number, o: any) => sum + (o.platformFee || o.totalPrice * 0.2), 0);
  const thisMonthCommission = commissionOrders
    .filter((o: any) => new Date(o.createdAt).getMonth() === new Date().getMonth())
    .reduce((sum: number, o: any) => sum + (o.platformFee || o.totalPrice * 0.2), 0);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-display font-bold text-dark-900 dark:text-cream-50 flex items-center gap-3">
             <PieChart className="w-10 h-10 text-emerald-500" /> Commission Tracker
          </h1>
          <p className="text-gray-500 mt-2">Track the 20% platform fee generated from all successful transactions.</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-dark-900 p-8 rounded-[40px] border border-emerald-500/20 shadow-card relative overflow-hidden">
           ...
           <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 flex items-center gap-2">
             <TrendingUp className="w-4 h-4 text-emerald-500" /> Total Lifetime Revenue
           </p>
           <p className="text-5xl font-accent font-bold text-emerald-500">Rs. {totalCommission.toLocaleString()}</p>
        </div>
        <div className="bg-white dark:bg-dark-900 p-8 rounded-[40px] border border-gold-400/20 shadow-card relative overflow-hidden">
           ...
           <p className="text-5xl font-accent font-bold text-gold-400">Rs. {thisMonthCommission.toLocaleString()}</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gold-400/5">
              {isLoading ? (
                ...
                filteredOrders.map((order: any) => {
                  const commission = order.platformFee || order.totalPrice * 0.2;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-6">
                        <span className="text-xs font-bold text-dark-900 dark:text-cream-50 uppercase tracking-widest">#{order.id.slice(-8)}</span>
                      </td>
                      <td className="p-6">
                      ...
                      <td className="p-6 text-right">
                        <p className="text-sm font-bold text-gray-500">Rs. {order.totalPrice.toLocaleString()}</p>
                      </td>
                      <td className="p-6 text-right">
                        <p className={`text-lg font-bold flex items-center justify-end gap-1 ${isConfirmed ? 'text-emerald-500' : 'text-amber-500'}`}>
                      ...
                      <td className="p-6 text-center">
                        <span className={`px-3 py-1 rounded-lg text-[9px] font-bold uppercase tracking-widest ${
                          isConfirmed ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 
