      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8">
        <div className="flex justify-between items-center mb-12">
          <div>
            ...
            <p className="text-gray-500">Review and process withdrawal requests from marketplace sellers.</p>
          </div>
              ...
              <tbody className="divide-y divide-gold-400/10">
                {isLoading ? (
                  ...
                  <tr key={p.id} className="hover:bg-gold-400/5 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-2">
                           <span className="text-[10px] bg-gold-400/10 text-gold-400 px-2 py-0.5 rounded-full font-bold">{p.method}</span>
                           <p className="text-xs text-gray-500 font-medium">{p.details}</p>
                        </div>
                        <p className="text-[9px] text-gray-400 uppercase tracking-tighter">{p.seller?.user?.email}</p>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1">
                        <span className={`inline-flex w-fit px-4 py-1.5 rounded-pill text-[10px] font-bold uppercase tracking-wider ${
                    ...
                    <td className="px-8 py-6 text-right">
                      {p.status === 'PENDING' && (
                        <div className="flex justify-end gap-2">
                          ...
                          <button 
                            onClick={() => { if(confirm('Reject this payout?')) handlePayout(p.id, 'REJECTED') }} 
                            title="Reject Payout"
          ...
          <div className="bg-white dark:bg-dark-900 w-full max-w-md rounded-[40px] p-10 relative shadow-2xl border border-gold-400/10">
            ...
            <div className="space-y-6">
              <div className="text-center">
                ...
                <h3 className="text-2xl font-display font-bold">Upload Payout Proof</h3>
                <p className="text-sm text-gray-500 mt-2">Upload the transfer receipt or screenshot to notify the seller.</p>
              </div>
                ...
                <label className="relative h-64 border-2 border-dashed border-gold-400/20 rounded-[32px] flex flex-col items-center justify-center gap-4 cursor-pointer hover:bg-gold-400/5 transition-all overflow-hidden">
                   ...
                   {proofFile ? (
                     <div className="relative w-full h-full">
                        <Image src={URL.createObjectURL(proofFile)} alt="Proof" fill className="object-cover" />
                     ...
                     <>
                       ...
                       <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Select Receipt Image</span>
                     </>
                   )}
                </label>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <button 
                  ...
                  onClick={() => selectedPayoutId && handlePayout(selectedPayoutId, 'COMPLETED')}
