  ...
  return (
    <div className="min-h-screen pt-20 md:pt-28 pb-16">
      <div className="max-w-6xl mx-auto px-4 md:px-6 lg:px-8">
        <h1 className="text-fluid-section font-display font-bold mb-6 md:mb-8 text-dark-900 dark:text-cream-50">User Directory</h1>
        ...
        <div className="bg-white dark:bg-dark-900 rounded-3xl md:rounded-[40px] border border-gold-400/10 shadow-card overflow-hidden">
                  ...
                  <tr key={u.id} className="hover:bg-gold-400/5 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-dark-900 dark:text-cream-50 text-sm">{u.name}</p>
                    ...
                    <td className="px-8 py-6">
                      <span className={`px-3 py-1 rounded-pill text-[9px] font-bold uppercase tracking-wider ${u.isActive ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
          ...
          <div className="md:hidden divide-y divide-gold-400/10">
             {isLoading ? (
               <div className="p-10 text-center animate-pulse text-gray-400 text-[10px] font-bold uppercase tracking-widest">Gathering records...</div>
             ) : users.length === 0 ? (
               <div className="p-10 text-center text-gray-400 text-[10px] font-bold uppercase tracking-widest">No users found</div>
             ) : users.map(u => (
               <div key={u.id} className="p-5 flex items-center justify-between gap-4 active:bg-gold-400/5 transition-all">
                  <div className="flex items-center gap-4 min-w-0">
                     ...
                     <div className="min-w-0">
                        <p className="font-bold text-dark-900 dark:text-cream-50 text-sm truncate">{u.name}</p>
                        ...
                        <div className="flex items-center gap-2">
                           ...
                           <span className={`px-2 py-0.5 rounded-md text-[8px] font-bold uppercase tracking-widest ${u.isActive ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                             {u.isActive ? 'Active' : 'Banned'}
                           </span>
                        </div>
                     </div>
