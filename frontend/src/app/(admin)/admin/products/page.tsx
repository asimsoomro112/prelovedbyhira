...
import { AlertTriangle, Ban, Check, Eye, Search, Trash2 } from "lucide-react";
										...
										>
											<td className="px-8 py-6">
												<div className="flex items-center gap-4">
													...
													<div>
														...
														<p className="text-[10px] text-gold-400 font-bold uppercase">
															{product.brand}
														</p>
													</div>
												</div>
											...
											<td className="px-8 py-6">
												<p className="text-sm font-medium">
													{product.seller?.user?.name || "Member"}
												</p>
											</td>
											<td className="px-8 py-6">
				...
				<div className="md:hidden divide-y divide-gold-400/10">
					{isLoading ? (
						...
						filteredProducts?.map((product: any) => (
							<div
								key={product.id}
								className="p-4 space-y-4 active:bg-gold-400/5 transition-all"
							>
				...
				{selectedProduct && (
					<>
						...
						<motion.div
							...
							className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100%-32px)] max-w-md bg-white dark:bg-dark-900 rounded-3xl md:rounded-[40px] p-6 md:p-8 z-[110] shadow-2xl border border-gold-400/10 space-y-6"
						>
							...
							<div className="flex flex-col sm:flex-row gap-3 pt-2 md:pt-4">
								<button
									onClick={() => setSelectedProduct(null)}
									className="w-full sm:flex-1 py-4 bg-gray-100 dark:bg-dark-800 rounded-2xl font-bold hover:bg-gray-200 transition-all text-sm min-h-[48px]"
								>
									Cancel
								</button>
								<button
