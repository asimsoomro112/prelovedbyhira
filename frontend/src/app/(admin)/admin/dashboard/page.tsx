		...
		return (
			<div className="h-screen flex flex-col items-center justify-center space-y-4">
				...
				<p className="text-gray-500">
					Please ensure you are logged in as an ADMIN and your Firestore
					database is accessible.
				</p>
				<button
					onClick={() => window.location.reload()}
					className="px-8 py-3 bg-gold-400 text-white rounded-pill font-bold"
				>
					Retry Connection
				</button>
			</div>
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
				<div className="space-y-1">
					...
					<p className="text-gray-500 text-sm">
						System overview and growth metrics.
					</p>
				</div>
				<div className="flex flex-row md:flex-row gap-3 w-full md:w-auto">
					<button className="flex-1 md:flex-none px-4 md:px-6 py-3 bg-white dark:bg-dark-800 border border-gold-400/10 rounded-pill font-bold text-xs md:text-sm shadow-soft active:scale-95 transition-all">
						Report
					</button>
					<button
						className="flex-1 md:flex-none px-4 md:px-6 py-3 bg-gold-400 text-white rounded-pill font-bold text-xs md:text-sm shadow-gold active:scale-95 transition-all"
						onClick={() => window.location.reload()}
					>
						Refresh
					</button>
			...
			<div className="grid lg:grid-cols-3 gap-8">
								...
								>
									...
									<Tooltip
										contentStyle={{
											borderRadius: "16px",
											border: "none",
											boxShadow: "0 10px 40px -10px rgba(196,163,90,0.2)",
										}}
										itemStyle={{
											fontSize: "12px",
											fontWeight: "bold",
										...
										>
											...
											<td className="px-8 py-6 text-sm font-medium">
												{order.seller?.user?.name ||
													order.product?.seller?.user?.name ||
													"Verified Merchant"}
											</td>
											...
											<td className="px-8 py-6">
												<span
													className={`px-3 py-1 rounded-pill text-[10px] font-bold ${order.status === "PAID" ? "bg-emerald-500/10 text-emerald-500" : "bg-amber-500/10 text-amber-500"}`}
												>
													{order.status}
												</span>
											</td>
							...
							{recentOrders?.map((order: any) => (
								...
								>
									<div className="flex justify-between items-start">
										...
										<span
											className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${order.status === "PAID" ? "bg-emerald-500/10 text-emerald-500" : "bg-amber-500/10 text-amber-500"}`}
										>
											{order.status}
										</span>
									</div>
				...
				<div className="space-y-6">
					<div className="bg-white dark:bg-dark-800 p-6 md:p-8 rounded-3xl md:rounded-[40px] shadow-soft border border-gold-400/5 space-y-6 md:space-y-8 text-dark-900 dark:text-white">
								...
								>
									<div className="flex items-center gap-4">
										<div className="relative w-12 h-12 rounded-full overflow-hidden border-2 border-gold-400/10 group-hover:border-gold-400 transition-all">
											<Image
												src={seller.user.avatar || "/placeholder.jpg"}
									...
									<div className="text-right">
										<p className="text-sm font-bold text-emerald-500">
											Rs. {Number(seller.totalEarnings).toLocaleString()}
										</p>
						...
						<Link
							href="/admin/sellers"
							className="block w-full py-4 border-2 border-gold-400/20 text-gold-400 text-center rounded-pill font-bold hover:bg-gold-400/10 transition-all text-sm"
						>
							View All Sellers
						</Link>
					</div>
					...
					<div className="p-6 md:p-8 bg-dark-900 text-white rounded-3xl md:rounded-[40px] shadow-gold-lg border border-gold-400/20 space-y-6">
						<h2 className="text-lg md:text-xl font-display font-bold text-gold-400">
							Global Alerts
						</h2>
...
function AdminStatCard({ label, value, change, icon, color }: any) {
	return (
		...
		>
			<div
				className={`w-12 h-12 md:w-14 md:h-14 ${color} bg-opacity-10 rounded-xl md:rounded-2xl flex items-center justify-center text-gold-400 group-hover:scale-110 transition-transform`}
			>
				{icon}
			</div>
			<div className="space-y-1">
				...
				<h3 className="text-xl md:text-3xl font-display font-bold">
					{value || "0"}
				</h3>
				<div className="flex items-center gap-1 text-[10px] font-bold text-emerald-500 pt-2">
					<ArrowUpRight className="w-3 h-3" /> {change}{" "}
					<span className="text-gray-400 ml-1">Live Status</span>
				</div>
			</div>
...
function AdminAlert({ icon, label, color = "text-gold-400", href = "#" }: any) {
	return (
		<Link
			href={href}
			className="flex items-center justify-between p-4 bg-white/5 rounded-2xl group hover:bg-white/10 transition-all cursor-pointer"
		>
			<div className="flex items-center gap-3">
