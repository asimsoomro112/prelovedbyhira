...
export default function EscrowManagerPage() {
	...
	const filteredOrders = escrowOrders.filter(
		(o: any) =>
			o.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
			o.product?.title.toLowerCase().includes(searchTerm.toLowerCase()),
	);

	const totalEscrowHeld = escrowOrders.reduce(
		(sum: number, o: any) => sum + (o.netAmount || o.totalPrice * 0.8),
		0,
	);
			...
			<div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
				<div>
					...
					<p className="text-gray-500 mt-2">
						Monitor and manage funds currently held in platform escrow.
					</p>
				</div>
				...
				<div className="bg-white dark:bg-dark-900 p-6 rounded-3xl border border-gold-400/20 shadow-soft w-full md:w-auto min-w-[250px]">
					...
					<p className="text-3xl font-accent font-bold text-gold-400">
						Rs. {totalEscrowHeld.toLocaleString()}
					</p>
				</div>
			</div>
						...
						<tbody className="divide-y divide-gold-400/5">
							{isLoading ? (
								...
								filteredOrders.map((order: any) => {
									const escrowAmount =
										order.netAmount || order.totalPrice * 0.8;
										...
										>
											<td className="p-6">
												<div className="flex items-center gap-4">
													...
													<div>
														<p className="font-bold text-sm text-dark-900 dark:text-cream-50">
											...
											<td className="p-6 text-right">
												<p className="text-lg font-bold text-gold-400">
													Rs. {escrowAmount.toLocaleString()}
												</p>
											</td>
												...
												{order.status === "DISPUTED" ? (
													...
													<button
														onClick={() => {
															if (
																confirm(
																	"Force release funds to seller? This overrides buyer confirmation.",
																)
															) {
