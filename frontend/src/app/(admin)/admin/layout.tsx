...
export default function AdminLayout({
	...
	if (!user || user.role !== "ADMIN") {
		return (
			<div className="min-h-screen bg-black flex items-center justify-center text-gold-400 font-bold uppercase tracking-widest">
					...
					<>
						...
						>
							<div className="flex items-center justify-between mb-10">
								...
								<button
									onClick={() => setIsMobileMenuOpen(false)}
									className="p-2 bg-gold-400/10 rounded-xl text-gold-400"
								>
									<X />
								</button>
							</div>
							...
							<button
								...
								className="mt-auto py-4 bg-red-500/10 text-red-500 rounded-2xl font-bold flex items-center justify-center gap-2 min-h-[48px]"
							>
			...
			<aside className="hidden lg:flex w-72 flex-col bg-white/80 dark:bg-zinc-900/50 backdrop-blur-3xl crystal-border rounded-[40px] shadow-2xl p-6 sticky top-4 h-[calc(100vh-32px)] transition-all">
                ...
                `}
							>
								...
								<span
									className={`w-5 h-5 ${isActive ? "text-gold-400" : "group-hover:scale-110 transition-transform"}`}
								>
									{item.icon}
								</span>
				...
				<div className="mt-auto p-4 space-y-4">
					<div className="p-4 bg-zinc-100 dark:bg-zinc-800/50 rounded-3xl border border-gold-400/10 flex items-center gap-3">
						...
						<div className="flex-1 min-w-0">
							<p className="text-xs font-bold truncate text-dark-900 dark:text-white">
								{user?.name}
							</p>
							<p className="text-[8px] font-bold text-gold-400 uppercase tracking-widest">
								Super Admin
							</p>
						</div>
					</div>
				...
				<header className="h-16 lg:h-20 bg-white/80 dark:bg-zinc-900/50 backdrop-blur-3xl crystal-border rounded-none lg:rounded-[32px] flex items-center justify-between px-4 lg:px-8 shadow-xl transition-all shrink-0 sticky top-0 z-20">
					<div className="flex items-center gap-4 flex-1">
						<button
							...
							className="lg:hidden p-3 bg-gold-400/10 text-gold-400 rounded-xl min-w-[48px] min-h-[48px] flex items-center justify-center active:scale-90 transition-transform"
						>
						...
						<div className="relative w-full max-w-md group hidden sm:flex">
							<div className="relative h-11 bg-zinc-100 dark:bg-black/20 rounded-xl border border-zinc-200 dark:border-white/5 flex items-center px-4 gap-3">
								...
								<input
									...
									className="bg-transparent border-none outline-none flex-1 text-xs font-medium text-dark-900 dark:text-white placeholder:text-zinc-400"
								/>
							</div>
						</div>
					</div>
