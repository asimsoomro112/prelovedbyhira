    ...
    <div className="p-4 md:p-6 lg:p-8 space-y-6 md:space-y-8 pb-32 lg:pb-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-fluid-section font-display font-bold text-dark-900 dark:text-cream-50">Moderation</h1>
      ...
      <div className="bg-white dark:bg-dark-800 rounded-3xl md:rounded-[40px] shadow-soft border border-gold-400/5 overflow-hidden">
        <div className="p-4 md:p-6 border-b border-gold-400/10 flex items-center justify-between">
           <div className="relative w-full max-w-md">
              ...
              <input placeholder="Search sellers..." className="w-full bg-cream-50 dark:bg-dark-900 rounded-2xl pl-10 md:pl-12 pr-4 py-3 text-sm outline-none border border-transparent focus:border-gold-400 transition-all min-h-[44px]" />
           </div>
        </div>
        ...
        <div className="md:hidden divide-y divide-gold-400/10">
          {isLoading ? (
            ...
            <div key={seller.id} className="p-5 space-y-4 active:bg-gold-400/5 transition-all">
               <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                     ...
                     <div className="min-w-0">
                        <p className="text-sm font-bold text-dark-900 dark:text-cream-50 truncate">{seller.user?.name || "Anonymous"}</p>
                        <p className="text-[10px] text-gray-500 truncate">{seller.user?.email || "No email"}</p>
                     </div>
                  </div>
                  <span className={`px-2 py-0.5 rounded-full text-[8px] font-bold uppercase tracking-widest ${getStatusStyle(seller.verificationStatus)}`}>
                    {seller.verificationStatus.replace("_", " ")}
                  </span>
               </div>
               <div className="flex justify-between items-center">
                  <div className="space-y-0.5">
                     <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">CNIC Number</p>
                     <p className="text-xs font-bold text-gray-600 dark:text-gray-300">{seller.cnicNumber || '---'}</p>
                  </div>
                  <button 
                    onClick={() => setSelectedSeller(seller)}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="fixed inset-x-0 bottom-0 md:inset-auto md:top-[5%] md:left-1/2 md:-translate-x-1/2 w-full md:max-w-5xl h-[95vh] md:h-auto md:max-h-[90vh] overflow-y-auto bg-white dark:bg-dark-900 rounded-t-[40px] md:rounded-[40px] p-6 md:p-8 lg:p-12 z-[110] shadow-2xl border-t md:border border-gold-400/10 space-y-8 md:space-y-10 scrollbar-none md:scrollbar-thin transition-colors">
               <div className="flex items-center justify-between sticky top-0 bg-white dark:bg-dark-900 z-10 pb-4 md:static">
                  <div className="flex items-center gap-3 md:gap-4">
                     ...
                     <div className="min-w-0">
                        <h2 className="text-xl md:text-2xl font-display font-bold truncate">{selectedSeller.user?.name || 'Seller Profile'}</h2>
                  ...
                  <button onClick={() => setSelectedSeller(null)} className="p-2 md:p-3 bg-gold-400/10 text-gold-400 rounded-full hover:bg-gold-400 hover:text-white transition-all"><X className="w-5 h-5 md:w-6 md:h-6" /></button>
               </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-6 gap-x-8">
                      ...
                      {selectedSeller.payoutDetails?.bankName && (
                        <div>
                            ...
                            <p className="text-sm font-bold text-emerald-500">{selectedSeller.payoutDetails.bankName}</p>
                        </div>
                      )}
                      <div>
