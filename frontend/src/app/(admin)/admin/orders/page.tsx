...
import { Package, ShoppingBag, User, Calendar, ShieldCheck, CheckCircle2, Eye, X, Upload, Camera, ImageIcon, AlertTriangle, MessageSquare } from 'lucide-react';
  ...
  const handleAction = async () => {
    ...
    try {
      if (modalMode === 'CONFIRM') {
        ...
        await api.put(`/orders/${selectedOrderId}/admin-reject`, { reason: rejectionReason });
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-12 gap-6">
          <div className="space-y-1">
            <h1 className="text-fluid-section font-display font-bold text-dark-900 dark:text-cream-50">Marketplace Orders</h1>
            <p className="text-gray-500 text-sm">Oversee neural audits and secure payments.</p>
          </div>
          <select 
            value={statusFilter} 
        ...
        <div className="bg-white dark:bg-dark-900 rounded-3xl md:rounded-[40px] border border-gold-400/10 shadow-card overflow-hidden">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {o.paymentRejected && (
                          <div className="flex flex-col gap-1">
                             ...
                             <p className="text-[8px] text-gray-400 italic leading-tight max-w-[150px]">Reason: {o.rejectionReason}</p>
                          </div>
                        )}
                      </div>
                    </td>
          ...
          <div className="md:hidden divide-y divide-gold-400/10">
            {isLoading ? (
               <div className="p-10 text-center text-gray-400 font-bold uppercase text-[10px] tracking-widest animate-pulse">Loading...</div>
            ) : orders.length === 0 ? (
               <div className="p-10 text-center text-gray-400 font-bold uppercase text-[10px] tracking-widest">No orders found</div>
            ) : orders.map(o => (
              <div key={o.id} className="p-5 space-y-4 active:bg-gold-400/5 transition-all">
                <div className="flex justify-between items-start">
                   <div className="space-y-1">
                      <p className="font-bold text-[10px] text-gold-400 font-mono">#{o.id.slice(-8).toUpperCase()}</p>
                      <p className="text-xs font-bold text-dark-900 dark:text-cream-50 leading-snug">{o.product?.title || 'Luxury Item'}</p>
                      <p className="text-[9px] text-gray-400 uppercase tracking-tighter">{new Date(o.createdAt).toLocaleDateString()}</p>
                   </div>
                ...
                <div className="grid grid-cols-2 gap-4 py-3 border-y border-gold-400/5">
                   <div>
                      <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">Buyer</p>
                      <p className="text-[11px] font-bold truncate">{o.buyer?.name || 'Unknown'}</p>
                   </div>
                   <div>
                      <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">Seller</p>
                      <p className="text-[11px] font-medium truncate text-gray-500">{o.seller?.user?.name || 'Verified'}</p>
                   </div>
                </div>
                ...
                <div className="flex justify-between items-end">
                   <div className="space-y-1">
                      ...
                      <p className="text-base font-accent font-bold text-gold-400">Rs. {o.totalPrice?.toLocaleString()}</p>
                   </div>
                   <div className="flex flex-col gap-2 items-end">
                      {o.paymentProofUrl && (
                        <a href={o.paymentProofUrl} target="_blank" className="flex items-center gap-1 text-[10px] font-bold text-gold-400">
          ...
          <div className="bg-white dark:bg-dark-900 w-full max-w-md rounded-3xl md:rounded-[40px] p-6 md:p-10 relative shadow-2xl border border-gold-400/10 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              <div className="text-center">
                <div className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl md:rounded-3xl flex items-center justify-center mx-auto mb-4 ${
                  modalMode === 'CONFIRM' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'
                }`}>
                   {modalMode === 'CONFIRM' ? <ShieldCheck className="w-8 h-8" /> : <AlertTriangle className="w-8 h-8" />}
                </div>
                <h3 className="text-xl md:text-2xl font-display font-bold">
                  {modalMode === 'CONFIRM' ? 'Confirm Payment' : 'Reject Payment'}
                </h3>
              ...
              {modalMode === 'REJECT' ? (
                ...
                <div className="bg-emerald-500/5 p-6 rounded-2xl border border-emerald-500/10">
                   <p className="text-sm text-center text-gray-600 dark:text-gray-300">
                     You are about to confirm that the customer's payment has been received in the vault. 
              ...
              <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                <button onClick={() => setIsModalOpen(false)} className="w-full sm:flex-1 py-4 bg-gray-100 dark:bg-dark-800 rounded-2xl font-bold text-gray-500 active:scale-95 transition-all min-h-[48px]">Cancel</button>
                <button 
                ...
                >
                  {isSubmitting ? 'Processing...' : modalMode === 'CONFIRM' ? 'Confirm Payment 🚀' : 'Reject Payment ❌'}
                </button>
              </div>
            </div>
          </div>
