...
export default function AdminDisputesPage() {
	...
	const { data: disputes, isLoading } = useQuery({
		...
		queryFn: async () => {
			...
			return activeTab === "ALL"
				? data
				: data.filter((d: any) => d.status === activeTab);
		},
	});
	...
	const resolveMutation = useMutation({
		mutationFn: async ({
			...
			const endpoint =
				action === "REFUND"
					? `/admin/disputes/${id}/approve-refund`
					: `/admin/disputes/${id}/reject`;
				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-gray-500">
							Manage order conflicts and protect marketplace integrity.
						</p>
					</div>
				</div>
				...
				{isLoading ? (
					Array(3)
						...
						.map((_, i) => (
							<div
								key={i}
								className="h-40 bg-white dark:bg-dark-800 rounded-[40px] animate-pulse"
							/>
						))
				) : disputes?.length === 0 ? (
					<div className="text-center py-20 bg-white dark:bg-dark-800 rounded-[40px] border border-gold-400/10">
						...
						<p className="text-gray-500 font-bold uppercase text-xs tracking-widest">
							No active disputes in {activeTab}
						</p>
					</div>
				) : (
					disputes?.map((dispute: any) => (
						...
						>
							<div className="text-center space-y-1 shrink-0">
								...
								<p className="text-2xl font-accent font-bold text-gold-400">
									Rs. {dispute.order.totalPrice}
								</p>
								<span
									className={`inline-block px-3 py-1 rounded-pill text-[8px] font-bold mt-2 ${getStatusStyle(dispute.status)}`}
								>
									{dispute.status}
								</span>
							</div>
							...
							<div className="flex-1 space-y-4">
								<div className="flex items-center gap-3">
									<span className="text-xs font-bold text-red-500 uppercase tracking-widest px-2 py-0.5 bg-red-500/10 rounded-md">
										{dispute.reason}
									</span>
								...
								<div className="flex items-center gap-12">
									<div className="flex items-center gap-2">
										<div className="w-6 h-6 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-500 text-[10px] font-bold">
											B
										</div>
										<span className="text-xs font-bold text-gray-500">
											Buyer: {dispute.order.customer.name}
										</span>
									</div>
									<div className="flex items-center gap-2">
										<div className="w-6 h-6 rounded-full bg-gold-400/10 flex items-center justify-center text-gold-400 text-[10px] font-bold">
											S
										</div>
										<span className="text-xs font-bold text-gray-500">
											Seller: {dispute.order.seller.user.name}
										</span>
									</div>
								</div>
							</div>
				...
				{selectedDispute && (
					<>
						...
						<motion.div
							...
							className="fixed top-[5%] left-1/2 -translate-x-1/2 w-full max-w-5xl max-h-[90vh] overflow-y-auto bg-white dark:bg-dark-900 rounded-[40px] p-8 lg:p-12 z-[110] shadow-2xl border border-gold-400/10 space-y-12 scrollbar-thin"
						>
							<div className="flex items-center justify-between">
								<div className="flex items-center gap-4">
									<AlertTriangle className="w-8 h-8 text-red-500" />
									<h2 className="text-2xl font-display font-bold">
										Review Dispute #{selectedDispute.id.slice(-8)}
									</h2>
								</div>
								<button
									onClick={() => setSelectedDispute(null)}
									className="p-3 bg-gold-400/10 text-gold-400 rounded-full hover:bg-gold-400 hover:text-white transition-all"
								>
									<X className="w-6 h-6" />
								</button>
							</div>
							...
							<div className="grid lg:grid-cols-3 gap-12">
								<div className="lg:col-span-2 space-y-10">
										...
										<div className="grid grid-cols-2 md:grid-cols-3 gap-4">
											{selectedDispute.evidence.map(
												(img: string, i: number) => (
													<div
														key={i}
														className="relative aspect-square rounded-3xl overflow-hidden border border-gold-400/10"
													>
														<Image
															src={img}
															alt="Evidence"
									...
									<div className="space-y-4">
										<h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2">
											<MessageSquare className="w-4 h-4" /> Description from
											Buyer
										</h3>
								...
								<div className="space-y-8">
									<div className="bg-white dark:bg-dark-950 p-8 rounded-[32px] border border-gold-400/10 space-y-6">
										...
										<div className="flex items-center gap-4">
											<div className="relative w-16 h-20 rounded-xl overflow-hidden">
												<Image
													src={selectedDispute.order.product.images[0]}
													alt="Product"
													fill
													className="object-cover"
												/>
											...
											<div>
												<p className="text-sm font-bold truncate w-40">
													{selectedDispute.order.product.title}
												</p>
												<p className="text-xs text-gold-400 font-bold">
													Rs. {selectedDispute.order.totalPrice}
												</p>
											</div>
										</div>
										<div className="space-y-3 pt-4 border-t border-gold-400/10">
											<InfoRow
												label="Status"
												value={selectedDispute.order.status}
											/>
											<InfoRow
												label="Buyer"
												value={selectedDispute.order.customer.name}
											/>
											<InfoRow
												label="Seller"
												value={selectedDispute.order.seller.user.name}
											/>
										</div>
									</div>
									...
									<div className="space-y-3">
										...
										>
											<ArrowUpRight className="w-4 h-4" /> Release Escrow to
											Seller
										</button>
...
function InfoRow({ label, value }: { label: string; value: string }) {
	return (
		<div className="flex justify-between items-center text-xs">
			...
			<span className="font-bold text-dark-900 dark:text-cream-50">
				{value}
			</span>
		</div>
	);
