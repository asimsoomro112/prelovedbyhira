	...
	return (
		<>
			...
			<div className="flex items-center justify-between gap-3 flex-wrap">
				...
				>
					<span
						className={`w-1.5 h-1.5 rounded-full animate-pulse ${(product.stock || 0) > 0 ? "bg-emerald-500" : "bg-red-500"}`}
					/>
					...
					<div className="flex items-center gap-1.5">
						<div className="flex">
							{[...Array(5)].map((_, i) => (
								<Star
									key={i}
									className={`w-4 h-4 ${i < 5 ? "fill-gold-400 text-gold-400" : "fill-gold-400/20 text-gold-400/20"}`}
								/>
							))}
						</div>
			...
			<div className="grid grid-cols-2 gap-3">
						...
						<div
							className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm shrink-0 ${
								cond.color === "emerald"
									...
									: cond.color === "gold"
										? "bg-gold-400/10 text-gold-500 border border-gold-400/20"
				...
				<div className="p-4 lg:p-5 bg-white dark:bg-dark-900 rounded-2xl border border-gold-400/10 shadow-sm space-y-2.5">
					...
					<div className="flex items-center justify-between gap-2">
						<p
							className={`font-accent font-bold text-dark-900 dark:text-cream-50 truncate ${
								(product.size || "").length > 6
									? "text-xl lg:text-2xl"
						...
						<div className="text-right shrink-0">
							...
							<p className="text-[9px] text-gold-400 font-bold mt-0.5">
								Size Guide →
							</p>
						</div>
					</div>
					{product.color && (
						<div className="flex items-center gap-2">
							<div
								className="w-4 h-4 rounded-full border border-gray-200 dark:border-gray-700 bg-gray-200"
			...
			{(product.stock || 0) > 1 && (
				<div className="flex items-center justify-between p-4 bg-white dark:bg-dark-900 rounded-2xl border border-gold-400/10 shadow-sm">
					...
					<div className="flex items-center gap-6">
						...
						<span className="text-xl font-accent font-bold w-4 text-center">
							{qty}
						</span>
						<button
