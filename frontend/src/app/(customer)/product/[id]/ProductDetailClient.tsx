...
/**
 * ProductDetailClient — Premium Preloved Marketplace PDP
...
function useLiveViewers(base = 5) {
	...
	useEffect(() => {
		const t = setInterval(
			() => setV((n) => Math.max(2, n + (Math.random() > 0.5 ? 1 : -1))),
			7000,
		);
...
function useInView(cb: (v: boolean) => void, deps: any[] = []) {
	...
	useEffect(() => {
		...
		const o = new IntersectionObserver(([e]) => cb(e.isIntersecting), {
			threshold: 0,
		});
...
const CONDITION: Record<
	string,
	{ grade: string; label: string; pct: number; desc: string; color: string }
> = {
	NEW: {
		grade: "A+",
		label: "Brand New",
		pct: 100,
		desc: "Never worn · Tags still attached",
		color: "emerald",
	},
	EXCELLENT: {
		grade: "A",
	...
	FAIR: {
		...
		label: "Fair",
		pct: 40,
		desc: "Visible wear · Still functional",
		color: "orange",
	},
};
...
export default function ProductDetailClient({
	...
	const [_hasSeenVideo, setHasSeenVideo] = useState(false);
	const [urgencyCount] = useState(() => Math.floor(Math.random() * 3) + 1);

	const videoRef = useRef<HTMLVideoElement>(null);
	const actionsRef = useInView(
		useCallback((v: boolean) => setSticky(!v), []),
		[],
	);

	// Use initialProduct if available, otherwise fetch
	...
	const product = res?.product || initialProduct;
	...
	const swipe = useSwipe(
		() =>
			...
			setImg((p) => (p < product.images.length - 1 ? p + 1 : 0)),
		() =>
			product?.images?.length > 1 &&
			setImg((p) => (p > 0 ? p - 1 : product.images.length - 1)),
	);
	...
	}, [id, product?.videoUrl, product, openVideoModal]);
	...
	if (!product)
		return (
			<div className="h-screen flex flex-col items-center justify-center gap-6 bg-gray-50 dark:bg-dark-950">
				...
				<Link
					href="/products"
					className="px-8 py-3.5 bg-gold-400 text-white rounded-2xl font-bold text-sm hover:scale-105 transition-all shadow-gold"
				>
					Browse Marketplace
				</Link>
			</div>
					...
					>
						...
						<div>
							<p className="text-[10px] font-bold text-dark-900 dark:text-cream-50 leading-tight">
								<span className="text-gold-400">{mobBuyer}</span> just saved
								this
							</p>
							<p className="text-[8px] text-gray-400 font-medium mt-0.5 flex items-center gap-1">
								<span className="w-1 h-1 bg-emerald-500 rounded-full" /> 3
								minutes ago
							</p>
						</div>
					</motion.div>
					...
					>
						...
						<div className="flex-1 min-w-0">
							<p className="text-[11px] font-bold text-dark-900 dark:text-cream-50">
								<span className="text-gold-400">{deskBuyer}</span> is viewing
								this piece
							</p>
						...
						<button
							onClick={() => setShowDeskFomo(false)}
							className="w-6 h-6 rounded-full bg-gray-100 dark:bg-dark-800 flex items-center justify-center text-gray-400 hover:text-gray-700 shrink-0"
						>
							<X className="w-3 h-3" />
						</button>
					</motion.div>
						...
						<div className="max-w-screen-xl mx-auto px-12 h-[68px] flex items-center justify-between w-full gap-6">
							<div className="flex items-center gap-4 min-w-0">
								...
								<div className="min-w-0">
									<p className="text-sm font-bold truncate max-w-[280px] text-dark-900 dark:text-cream-50">
										{product.title}
									</p>
									<div className="flex items-center gap-2 mt-0.5">
										<span className="text-base font-accent font-bold text-gold-400">
							...
							<div className="flex items-center gap-3 shrink-0">
								<div className="flex items-center gap-2 text-[10px] font-bold text-red-500">
									<Flame className="w-3.5 h-3.5 animate-pulse" /> {viewers}{" "}
									viewing · {urgencyCount} left
								</div>
								<button
									onClick={() => onAddBag(qty)}
									className="h-10 px-5 border-2 border-gold-400/40 text-gold-500 rounded-xl font-bold text-xs hover:border-gold-400 hover:bg-gold-400/5 transition-all flex items-center gap-2"
								>
			...
			<main className="min-h-screen bg-gray-50 dark:bg-dark-950 pb-36 lg:pb-20">
				<div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-12 py-6 lg:py-12">
					...
					<nav
						...
						className="flex items-center gap-2 mb-5 lg:mb-8 text-[10px] font-bold uppercase tracking-widest text-gray-400"
					>
						<Link
							href="/products"
						...
						<Link
							...
							className="hover:text-gold-400 transition-colors text-gold-400/70"
						>
							{product.category}
						</Link>
						<ChevronRight className="w-3 h-3" />
					...
					<div className="lg:hidden flex items-center justify-between mb-4 px-0.5">
						<div className="flex items-center gap-2">
							<span className="flex items-center gap-1.5 bg-red-500/10 border border-red-500/20 rounded-full px-3 py-1.5 text-[9px] font-black text-red-500 uppercase tracking-widest">
								<Flame className="w-3 h-3 animate-pulse" /> {urgencyCount} of{" "}
								{urgencyCount} Left
							</span>
					...
					<div className="hidden lg:flex items-center justify-between mb-8 px-6 py-4 bg-white dark:bg-dark-900 rounded-2xl border border-gold-400/10 shadow-sm">
						<div className="flex items-center gap-6">
							<span className="flex items-center gap-2 bg-red-500/8 border border-red-500/15 rounded-full px-4 py-2 text-[10px] font-black text-red-500 uppercase tracking-widest">
								<Flame className="w-3.5 h-3.5 animate-pulse" /> Only{" "}
								{urgencyCount} Remaining in Vault
							</span>
							<div className="flex items-center gap-2.5">
								<div className="flex -space-x-2">
									{[...Array(Math.min(viewers, 4))].map((_, i) => (
										<div
											key={i}
											className="w-6 h-6 rounded-full bg-gradient-to-br from-gold-300 to-gold-500 border-2 border-white dark:border-dark-900 shadow-sm"
										/>
									))}
								</div>
								<p className="text-[11px] text-gray-500 font-medium">
									<span className="font-black text-dark-900 dark:text-cream-50">
										{viewers}
									</span>{" "}
						...
						<div className="flex items-center gap-5">
							<span className="flex items-center gap-1.5 text-[10px] font-bold text-emerald-500 uppercase tracking-widest">
								<Timer className="w-3.5 h-3.5" /> Ships in 24h
							</span>
							<span className="text-gray-200 dark:text-gray-700">|</span>
							<span className="flex items-center gap-1.5 text-[10px] font-bold text-gold-400 uppercase tracking-widest">
						...
						<AnimatePresence mode="wait">
									...
									<div className="grid sm:grid-cols-2 gap-6 pt-4 border-t border-gold-400/10">
										{[
											...
											<div key={i} className="flex items-center gap-3">
												<span className="text-[9px] font-black text-gray-400 uppercase tracking-widest w-20 shrink-0">
													{r.label}
												</span>
												<span className="text-sm font-bold text-dark-900 dark:text-cream-50">
													{r.value}
												</span>
											</div>
										))}
							...
							{tab === "condition" && (
								...
								>
									<div className="grid sm:grid-cols-2 gap-4">
										{[
											...
											},
											{
												...
												value: product.originalPacking ? "Yes" : "No",
												ok: !!product.originalPacking,
											},
											{
												label: "Invoice Available",
												value: product.invoiceAvailable ? "Yes" : "No",
												ok: !!product.invoiceAvailable,
											},
											{
												label: "Any Alterations",
												value: product.isAltered ? "Yes" : "None",
												ok: !product.isAltered,
											},
										].map((r, i) => (
											<div
												key={i}
												className={`flex items-center justify-between p-4 rounded-2xl border ${r.ok ? "bg-emerald-500/5 border-emerald-500/15" : "bg-red-500/5 border-red-500/15"}`}
											>
												<span className="text-xs font-bold text-dark-900 dark:text-cream-50">
													{r.label}
												</span>
												<span
													className={`flex items-center gap-1.5 text-xs font-bold ${r.ok ? "text-emerald-500" : "text-red-400"}`}
												>
													{r.ok ? (
									...
									<p className="text-xs text-gray-500 font-medium mt-6 leading-relaxed">
										All items are personally inspected and graded by our team.
										Condition grades follow industry-standard guidelines for
										preloved fashion.
									</p>
								</motion.div>
							)}
									...
									{[
										{
											...
											body: "Your order is dispatched the next business day after payment confirmation.",
										},
										{
											icon: <Lock className="w-5 h-5 text-gold-400" />,
											title: "Escrow Protection",
											body: "Your payment is held securely. Funds only release when you confirm receipt and authenticity.",
										},
										{
											icon: <Package className="w-5 h-5 text-gold-400" />,
											title: "Nationwide Delivery",
											body: "Insured door-to-door shipping across Pakistan in 3–5 business days.",
										},
										{
											icon: <RotateCcw className="w-5 h-5 text-gray-400" />,
											title: "7-Day Return Policy",
											body: "Not satisfied? Return within 7 days for a full refund — no questions asked.",
										},
									].map((s, i) => (
										<div
											key={i}
											className="flex gap-4 p-4 bg-white dark:bg-dark-900 rounded-2xl border border-gold-400/10"
										>
											<div className="w-10 h-10 rounded-xl bg-gray-50 dark:bg-dark-800 flex items-center justify-center shrink-0">
												{s.icon}
											</div>
											<div>
							...
							{tab === "reviews" && (
								<motion.div
									...
									exit={{ opacity: 0, y: -8 }}
									transition={{ duration: 0.3 }}
									className="text-center py-16 bg-white dark:bg-dark-900 rounded-[32px] border border-gold-400/10 shadow-sm"
								>
									<div className="w-20 h-20 bg-gold-400/10 text-gold-400 rounded-full flex items-center justify-center mx-auto mb-6">
										<Star className="w-10 h-10" />
									</div>
									<h3 className="text-2xl font-display font-bold text-dark-900 dark:text-cream-50">
										No Reviews Yet
									</h3>
									<p className="text-gray-500 text-sm mt-3 max-w-xs mx-auto">
										Only verified buyers can leave reviews after a successful
										transaction. Your feedback will help build seller trust.
									</p>
								</motion.div>
							)}
						</AnimatePresence>
					...
					{related && related.length > 0 && (
						<div className="mt-24 lg:mt-32 pt-16 border-t border-gold-400/10">
							<div className="flex items-end justify-between mb-12">
								<div>
									<p className="text-[10px] font-black uppercase tracking-[0.4em] text-gold-500 mb-3">
										Collector's Choice
									</p>
									<h3 className="text-3xl lg:text-5xl font-display font-bold text-dark-900 dark:text-cream-50">
										Similar{" "}
										<span className="italic text-gold-400">Treasures</span>
									</h3>
								</div>
								<Link
									href="/products"
									className="group flex items-center gap-2 text-xs font-black uppercase tracking-widest text-gold-500 hover:text-gold-600 transition-colors"
								>
									View All Collection{" "}
									<ArrowRight
										size={14}
								...
								{related.map((p: any) => (
									...
									>
										<Link href={`/product/${p.id}`} className="block group">
											<div className="relative aspect-[3/4] rounded-3xl overflow-hidden bg-white dark:bg-dark-900 border border-gold-400/5 shadow-sm group-hover:shadow-xl group-hover:shadow-gold-400/10 transition-all">
												<Image
											...
											<div className="mt-5 space-y-1">
												...
												<h4 className="font-bold text-dark-900 dark:text-cream-50 group-hover:text-gold-500 transition-colors truncate">
													{p.title}
												</h4>
												<p className="font-accent font-bold text-lg text-dark-900 dark:text-cream-50">
				...
				<div className="bg-white dark:bg-dark-950 border-t border-gold-400/15 px-4 pt-3 pb-[env(safe-area-inset-bottom,16px)] pb-4">
					<div className="flex items-center justify-between mb-3">
						<div>
							...
							<p className="text-2xl font-accent font-bold text-gold-400 leading-none">
								Rs. {(product.sellingPrice || 0).toLocaleString()}
							</p>
						</div>
						<div className="flex items-center gap-2">
					...
					<div className="flex gap-2.5">
						...
						>
							...
							>
								...
								{(product.stock || 0) > 0 ? "Buy Now" : "Sold Out"}
							</Link>
						</motion.div>
					</div>
				</div>
				...
				{lightbox && (
					...
					>
						...
						<div className="relative w-full h-full max-w-5xl flex items-center justify-center">
							...
							>
								<Image
									src={product.images?.[img] || "/placeholder.jpg"}
									alt="HD"
									fill
									sizes="100vw"
							...
							{product.images?.length > 1 && (
								<>
									<button
										...
										className="absolute left-0 lg:-left-16 w-14 h-14 rounded-full bg-white/8 hover:bg-gold-400 text-white flex items-center justify-center transition-all"
									>
									...
									<button
										...
										className="absolute right-0 lg:-right-16 w-14 h-14 rounded-full bg-white/8 hover:bg-gold-400 text-white flex items-center justify-center transition-all"
									>
						...
						<div className="mt-8 flex gap-3 overflow-x-auto scrollbar-none px-4 shrink-0">
							{product.images?.map((src: string, i: number) => (
								<button
									key={i}
									onClick={() => setImg(i)}
									className={`relative w-16 h-20 rounded-xl overflow-hidden shrink-0 transition-all ${i === img ? "ring-2 ring-gold-400 scale-110" : "opacity-35 hover:opacity-70"}`}
								>
									<Image
										src={src}
										alt=""
							...
							>
								...
								<div className="p-6 lg:p-8 bg-dark-900 flex flex-col sm:flex-row items-center justify-between gap-6 border-t border-white/5">
									<div className="space-y-1 text-center sm:text-left">
										<h3 className="text-xl font-display font-bold text-cream-50 leading-tight">
											{product.title}
										</h3>
										<p className="text-xs text-gray-500 font-medium uppercase tracking-widest">
											Spatial Luxury · Vetted Condition · Rs.{" "}
