...
export default function ProductDetailClient({ product: initialProduct, related = [] }: { product: any; related?: any[] }) {
  const { id }      = useParams();
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-12 py-6 lg:py-12">
          ...
          {related && related.length > 0 && (
            <div className="mt-24 lg:mt-32 pt-16 border-t border-gold-400/10">
                ...
                <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gold-500 mb-3">Collector's Choice</p>
                    <h3 className="text-3xl lg:text-5xl font-display font-bold text-dark-900 dark:text-cream-50">Similar <span className="italic text-gold-400">Treasures</span></h3>
                </div>
                <Link href="/products" className="group flex items-center gap-2 text-xs font-black uppercase tracking-widest text-gold-500 hover:text-gold-600 transition-colors">
                ...
                {related.map((p: any) => (
                    ...
                    <Link href={`/product/${p.id}`} className="block group">
                        ...
                        <Image src={p.images?.[0] || "/placeholder.jpg"} alt={p.title} fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                        </div>
                        <div className="mt-5 space-y-1">
                        <p className="text-[10px] font-black uppercase tracking-widest text-gold-500">{p.brand}</p>
                        <h4 className="font-bold text-dark-900 dark:text-cream-50 group-hover:text-gold-500 transition-colors truncate">{p.title}</h4>
                        <p className="font-accent font-bold text-lg text-dark-900 dark:text-cream-50">Rs. {p.sellingPrice?.toLocaleString()}</p>
                        </div>
                    </Link>
                    </motion.div>
                ))}
