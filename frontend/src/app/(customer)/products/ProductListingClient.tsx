...
export default function ProductListingClient({
	...
	const toggleFilter = (
		type: "category" | "condition" | "size" | "brand",
		value: string,
	) => {
		setFilters((prev) => {
		...
		<div className="max-w-screen-xl mx-auto px-4 md:px-6 py-4 md:py-8">
						...
						<div className="relative group">
							...
							>
								{SORT_OPTIONS.find((opt) => opt.id === filters.sortBy)?.label ||
									"Sort By"}
								<ChevronDown
									className={`w-4 h-4 text-gold-400 transition-transform duration-300 ${isSortOpen ? "rotate-180" : ""}`}
								/>
							</button>
					...
					<div className="flex flex-wrap gap-2 mt-3">
						{filters.category.map((c) => (
							<button
								...
								className="flex items-center gap-1 px-3 py-1.5 bg-gold-400/10 text-gold-400 rounded-full text-[11px] font-bold min-h-[32px]"
							>
						...
						<button
							...
							className="px-3 py-1.5 text-red-400 text-[11px] font-bold min-h-[32px]"
						>
							Clear All
						</button>
					</div>
			...
			<div className="flex gap-8 lg:gap-12">
				...
				<div className="flex-1 space-y-6">
					<div
						className={`grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 lg:gap-6 transition-opacity duration-300 ${isFetching ? "opacity-40" : "opacity-100"}`}
					>
						{isLoading && !data
							? Array(8)
									.fill(0)
					...
					<div className="fixed inset-0 z-[200] lg:hidden">
						...
						>
							...
							<div className="flex items-center justify-between px-5 py-3 border-b border-gold-400/10">
								...
								<div className="flex items-center gap-3">
									{activeFilterCount > 0 && (
										<button
											onClick={clearAllFilters}
											className="text-xs font-bold text-red-400 min-h-[44px] flex items-center"
										>
											Reset
										</button>
									)}
							...
							<div className="p-4 border-t border-gold-400/10">
								...
								>
									Show Results{" "}
									{data?.pages[0]?.pagination?.totalProducts
										? `(${data.pages[0].pagination.totalProducts})`
										: ""}
								</button>
							</div>
...
function _FilterItem({
	...
	return (
		<button
			...
			aria-pressed={active}
		>
			<span className="text-sm font-medium transition-colors group-hover:text-gold-400">
				{label}
			</span>
			<div
				className={`w-6 h-6 rounded-md border-2 transition-all flex items-center justify-center ${active ? "border-gold-400 bg-gold-400 text-white" : "border-gold-400/20 group-hover:border-gold-400/40"}`}
			>
