...
import { Star, ShieldCheck, MapPin, Package, Clock, ChevronRight, User } from "lucide-react";
...
export default function PublicShopPage({ params }: { params: Promise<{ sellerId: string }> }) {
  ...
  if (isError || !shop) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-6">
        ...
        <p className="text-gray-500 mt-2">This seller profile doesn't exist or is currently inactive.</p>
        <Link href="/products" className="mt-8 px-8 py-4 bg-gold-400 text-white rounded-pill font-bold shadow-gold">
          Browse Marketplace
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <div className="relative h-[300px] md:h-[400px]">
        {shop.coverImage ? (
          <Image src={shop.coverImage} alt="Cover" fill className="object-cover" priority />
        ) : (
        ...
        <div className="absolute bottom-0 left-0 w-full translate-y-1/2 px-6 lg:px-12">
           <div className="max-w-screen-xl mx-auto flex flex-col md:flex-row items-center md:items-end gap-6">
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-cream-50 dark:border-dark-900 bg-white dark:bg-dark-800 shadow-card overflow-hidden relative shrink-0">
                {shop.avatar ? (
                  <Image src={shop.avatar} alt={shop.shopName} fill className="object-cover" />
                ) : (
              ...
              <div className="flex-1 text-center md:text-left mb-2 md:mb-6">
                 <div className="flex items-center justify-center md:justify-start gap-3">
                   ...
                   {shop.isVerified && <ShieldCheck className="w-6 h-6 text-emerald-400 drop-shadow-md" />}
                 </div>
                 <p className="text-gray-500 md:text-gray-300 mt-2 max-w-xl text-sm leading-relaxed drop-shadow-sm">{shop.bio}</p>
              </div>
              ...
              <div className="flex items-center gap-4 mb-2 md:mb-6">
                    ...
                    <div className="flex items-center justify-center gap-1 text-gold-400 font-bold text-xl">
                       <Star className="w-5 h-5 fill-current" /> {(shop.rating || 5).toFixed(1)}
                    </div>
                 ...
                 <div className="bg-white dark:bg-dark-800 px-6 py-3 rounded-2xl shadow-soft border border-gold-400/10 text-center">
                    <div className="flex items-center justify-center gap-1 text-dark-900 dark:text-cream-50 font-bold text-xl">
                       <Package className="w-5 h-5 text-gray-400" /> {shop.totalSales || 0}
                    </div>
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-1">Sales</p>
                 </div>
              </div>
           </div>
        </div>
      ...
      <div className="max-w-screen-xl mx-auto px-6 mt-32 md:mt-24 space-y-16">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="p-6 bg-cream-100 dark:bg-dark-800 rounded-3xl flex items-center gap-4">
               ...
               <div>
                  <p className="text-xs text-gray-500 font-bold uppercase">Escrow Protected</p>
            ...
            <div className="p-6 bg-cream-100 dark:bg-dark-800 rounded-3xl flex items-center gap-4">
               ...
               <div>
                  ...
                  <p className="font-bold">{new Date(shop.joinedAt || new Date()).getFullYear()}</p>
               </div>
            </div>
         </div>
         ...
         <div className="space-y-8">
            <h2 className="text-2xl font-display font-bold flex items-center gap-2 border-b border-gold-400/10 pb-4">
               Shop Collection <span className="text-sm font-normal text-gray-500 ml-auto">{shop.products?.length || 0} items</span>
            </h2>
            ...
            {!shop.products || shop.products.length === 0 ? (
               ...
               <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {shop.products.map((product: any) => (
                     <Link key={product.id} href={`/product/${product.id}`} className="group bg-white dark:bg-dark-800 rounded-[32px] overflow-hidden shadow-soft border border-gold-400/10 hover:shadow-card transition-all">
                        ...
                        <div className="p-5 space-y-1">
                           <h3 className="font-bold text-dark-900 dark:text-cream-50 truncate">{product.title}</h3>
                           <p className="text-xs text-gray-500 truncate">{product.brand}</p>
                           <p className="text-lg font-bold text-gold-400 pt-2">Rs. {(product.sellingPrice || 0).toLocaleString()}</p>
                        </div>
                     </Link>
                  ))}
               </div>
