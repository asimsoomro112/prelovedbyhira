...
export default function NotificationsPage() {
	...
	const getBgStyle = (type: string, isRead: boolean) => {
		if (isRead)
			return "bg-white dark:bg-dark-900 border-gray-100 dark:border-dark-800 opacity-60";

		switch (type) {
			case "ORDER":
				return "bg-blue-500/5 border-blue-500/20 shadow-soft";
			case "WISHLIST":
				return "bg-rose-500/5 border-rose-500/20 shadow-soft";
			case "DISPUTE":
				return "bg-red-500/5 border-red-500/20 shadow-soft";
			case "MESSAGE":
				return "bg-emerald-500/5 border-emerald-500/20 shadow-soft";
			default:
				return "bg-gold-400/5 border-gold-400/20 shadow-soft";
		}
	};
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-gray-500 text-sm mt-1">
						Stay updated with your orders and wishlist.
					</p>
				</div>
						...
						>
							...
							<div className="flex-1 space-y-1">
								<div className="flex justify-between items-start gap-4">
									<h3
										className={`font-bold ${notif.isRead ? "text-gray-600 dark:text-gray-400" : "text-dark-900 dark:text-cream-50"}`}
									>
										{notif.title}
									</h3>
								...
								<p
									className={`text-sm ${notif.isRead ? "text-gray-500" : "text-gray-600 dark:text-gray-300"}`}
								>
									{notif.message}
								</p>
							</div>
