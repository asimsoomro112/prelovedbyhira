...
export default function CartPage() {
	...
	const uniqueSellers = products
		? Array.from(new Set(products.map((p: any) => p.sellerId)))
		: [];
				...
				<div className="flex-1 space-y-6 lg:space-y-10">
					<div className="space-y-1">
						<h1 className="text-fluid-section font-display font-bold text-dark-900 dark:text-cream-50">
							Your <span className="italic text-gold-400">Bag.</span>
						</h1>
						<p className="text-dark-500 dark:text-gray-400 text-sm font-medium">
							{items.length} premium preloved items selected.
						</p>
					</div>
								...
								>
									...
									<div className="space-y-2">
										...
										<p className="text-gray-500 text-sm max-w-xs mx-auto">
											Discover authentic preloved luxury and fill it with style.
										</p>
									</div>
									<Link
										href="/products"
										className="inline-flex h-14 px-8 items-center bg-gold-400 text-white rounded-2xl font-bold shadow-gold hover:scale-105 active:scale-95 transition-all min-h-[52px]"
									>
										Start Shopping
									</Link>
								</motion.div>
											...
											<div className="flex-1 flex flex-col justify-between py-0.5 min-w-0">
												<div className="flex justify-between items-start gap-2">
													<div className="space-y-0.5 min-w-0">
														...
														<h3 className="text-sm md:text-lg font-bold text-dark-900 dark:text-cream-50 line-clamp-2">
															{product.title}
														</h3>
												...
												<div className="flex justify-between items-end mt-3">
													...
													<div className="flex items-center bg-cream-50 dark:bg-dark-700 rounded-xl p-1 gap-1">
														...
														<button
															onClick={() =>
																item?.id && updateItem(item.id, (item.quantity || 1) + 1)
															}
															className="w-9 h-9 rounded-lg flex items-center justify-center hover:bg-gold-400/10 text-gold-400 active:scale-90 transition-all"
													...
													<p className="text-lg md:text-2xl font-accent font-bold text-gold-400">
				...
				>
					<div className="flex items-center justify-between mb-2">
						<div>
							...
							<p className="text-xl font-accent font-bold text-gold-400">
								Rs. {total.toLocaleString()}
							</p>
						</div>
						<div className="text-right">
							<p className="text-[10px] text-gray-400">
								{items.length} items + shipping
							</p>
						</div>
					</div>
					<Link
		...
		<>
			<div className="glass-ultra crystal-border rounded-[48px] p-10 space-y-8 shadow-gold-3d">
				...
				<div className="space-y-4">
					<div className="flex justify-between text-sm font-medium text-gray-500">
						<span>Subtotal</span>
						<span className="text-dark-900 dark:text-cream-50 font-bold">
					...
					<div className="flex justify-between items-end">
						...
						<span className="text-3xl font-accent font-bold text-gold-400">
							Rs. {total.toLocaleString()}
						</span>
					</div>
				</div>
			...
			<div className="p-6 glass-crystal crystal-border rounded-3xl text-center">
				...
				<p className="text-xs text-gray-500">
					Our support agents are live 24/7.{" "}
					<span className="text-gold-400 underline">Chat Now</span>
				</p>
			</div>
		</>
	);
