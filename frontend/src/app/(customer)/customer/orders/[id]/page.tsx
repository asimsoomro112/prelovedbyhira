...
export default function OrderDetailsPage() {
	...
	const [_isReviewing, _setIsReviewing] = useState(false);
	...
	const handleReupload = async () => {
		...
		try {
			...
			toast.success(
				"Receipt re-uploaded successfully! AI verification in progress.",
			);
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
				<div className="space-y-2">
					<Link
						...
						className="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest hover:text-gold-400 transition-colors"
					>
					...
					<p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">
						Manifest ID: #{order.id.slice(-12)}
					</p>
				</div>
				<div
					className={`px-6 py-3 rounded-2xl text-xs font-bold uppercase tracking-widest flex items-center gap-2 ${getStatusStyle(order.status)}`}
				>
				...
				>
					...
					<div className="space-y-1">
						...
						<p className="text-xs opacity-90 leading-relaxed">
							Receipt submitted successfully. Please wait for admin to verify
							your payment from the bank records.
						</p>
					</div>
				</motion.div>
			)}
					...
					<div className="glass-ultra crystal-border rounded-3xl md:rounded-[40px] p-5 md:p-8 space-y-6 md:space-y-8">
						<div className="flex gap-4 md:gap-8">
							...
							<div className="space-y-1 md:space-y-2">
								<p className="text-[9px] md:text-[10px] font-bold text-gold-400 uppercase tracking-widest">
									{order.product?.brand}
								</p>
								...
								<p className="text-[10px] md:text-sm text-gray-500 font-medium">
									Cond: {order.product?.condition} • Size: {order.product?.size}
								</p>
						...
						<div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gold-400/5">
							...
							{order.status === "SHIPPED" && (
								<Link
									...
									className="w-full h-14 bg-gold-400 text-white rounded-2xl font-bold shadow-gold flex items-center justify-center gap-2 active:scale-95 transition-all"
								>
							...
							{order.status === "CONFIRMED" && (
								...
								>
									...
									{order.reviewed ? "Review Submitted" : "Write a Review"}
								</button>
							)}
							{["PAID", "SHIPPED"].includes(order.status) && (
								<Link
									href={`/customer/orders/${order.id}/dispute`}
									className="w-full text-center text-[10px] font-bold text-red-500 uppercase tracking-widest hover:underline py-2 active:scale-95"
								>
				...
				<div className="space-y-8">
					<div className="glass-ultra rounded-[40px] border border-gold-400/10 overflow-hidden shadow-gold-3d">
						<div className="bg-gold-400 p-8 text-white">
							<h3 className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-80 mb-2">
								Order Summary
							</h3>
							<p className="text-3xl font-accent font-bold">
							...
							<div className="space-y-4">
								<div className="flex justify-between text-sm">
									<span className="text-gray-500">
										Item Price {order.quantity > 1 && `(×${order.quantity})`}
									</span>
										...
										{order.quantity > 1 && (
											<p className="text-[10px] text-gray-400 font-bold uppercase mt-1">
												...
												{order.unitPrice?.toLocaleString() ||
													(order.sellingPrice || 0).toLocaleString()}
											</p>
										)}
									</div>
								</div>
								...
								<div className="flex justify-between items-center">
									...
									<span className="text-2xl font-accent font-bold text-gold-400">
										Rs. {order.totalPrice.toLocaleString()}
									</span>
								</div>
							</div>
								...
								<div className="flex items-start gap-3">
									...
									<div>
										...
										<p className="text-[10px] text-gray-500 leading-relaxed">
											Your funds are held safely. We only release them once you
											verify the item.
										</p>
									</div>
								</div>
					...
					<div className="p-8 glass-crystal rounded-[32px] border border-gold-400/10 text-center space-y-4">
						...
						<p className="text-xs text-gray-500 leading-relaxed">
							Our concierge team is ready to help with any order inquiries.
						</p>
						<button
					...
					>
						...
						<div className="space-y-6">
									...
									{receiptFile ? (
										...
										<>
											...
											<div className="text-center">
												...
												<span className="text-[9px] text-gray-400/60 mt-1 block">
													JPG, PNG or PDF (Capture)
												</span>
											</div>
										</>
									)}
							...
							<div className="flex gap-4">
								<button
									onClick={() => setIsReuploadModalOpen(false)}
									className="flex-1 py-4 bg-gray-100 dark:bg-dark-800 rounded-2xl font-bold text-gray-500 active:scale-95 transition-all"
								>
									Cancel
								</button>
								<button
					...
					>
						...
						<div className="space-y-8">
							<div className="text-center">
								...
								<h3 className="text-2xl font-display font-bold">
									Share Your <span className="text-gold-400">Experience</span>
								</h3>
								<p className="text-sm text-gray-500 mt-2">
									Rate your luxury purchase and help other members of the
									community.
								</p>
							</div>
							...
							<div className="flex justify-center gap-2">
								{[1, 2, 3, 4, 5].map((s) => (
									...
									>
										<Star
											className={`w-10 h-10 ${s <= rating ? "text-gold-400 fill-gold-400" : "text-gray-200 dark:text-dark-700"}`}
										/>
									</button>
								))}
							</div>
