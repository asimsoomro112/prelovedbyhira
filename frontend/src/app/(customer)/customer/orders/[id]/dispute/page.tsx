...
const disputeSchema = z.object({
  reason: z.enum(["ITEM_NOT_RECEIVED", "ITEM_NOT_AS_DESCRIBED", "DAMAGED_ITEM", "WRONG_ITEM"]),
...
export default function DisputePage() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<DisputeValues>({
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12 space-y-12">
      <div className="flex items-center justify-between">
        <Link href={`/customer/orders/${id}`} className="flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-gold-400">
      ...
      <div className="bg-white dark:bg-dark-900 rounded-[40px] shadow-card border border-gold-400/10 overflow-hidden">
        <div className="p-8 lg:p-12 space-y-10">
           <div className="text-center space-y-2">
              ...
              <p className="text-gray-500">Tell us what went wrong. We&apos;re here to help resolve this.</p>
           </div>
           ...
           <div className="p-6 bg-cream-50 dark:bg-dark-800 rounded-3xl flex items-center gap-4 border border-gold-400/5">
              <div className="relative w-16 h-20 rounded-xl overflow-hidden shrink-0">
                 <Image src={order.product.images[0]} alt="Product" fill className="object-cover" />
              </div>
              <div>
                 <p className="text-sm font-bold">{order.product.title}</p>
                 <p className="text-xs text-gray-400">Seller: {order.seller.user.name}</p>
                 <p className="text-xs font-bold text-gold-400 mt-1">Rs. {order.totalPrice}</p>
              </div>
           </div>

           <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              <div className="space-y-2">
                 <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Reason for Dispute</label>
                 <select {...register("reason")} className="w-full bg-cream-50 dark:bg-dark-800 border-2 border-gold-400/10 rounded-2xl px-6 py-4 outline-none focus:border-gold-400 transition-all font-bold">
              ...
              <div className="space-y-4">
                 <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Evidence Photos (Up to 5)</label>
                 <div className="grid grid-cols-3 md:grid-cols-5 gap-4">
                    ...
                    {previews.map((src, i) => (
                      <div key={i} className="relative aspect-square rounded-2xl overflow-hidden border border-gold-400/10 group">
