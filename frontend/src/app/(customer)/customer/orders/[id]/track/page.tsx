  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12 space-y-12">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Order Tracking #{order.id.slice(-8)}</span>
      </div>

      <div className="bg-white dark:bg-dark-900 rounded-[40px] p-8 lg:p-12 shadow-card border border-gold-400/10 space-y-12">
        <div className="text-center space-y-2">
           <h1 className="text-3xl font-display font-bold">Where&apos;s your order?</h1>
           <p className="text-gray-500">Your style is on its way to your doorstep.</p>
        </div>
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              <div key={step.id} className="relative flex gap-8 group">
                <div className={`w-11 h-11 rounded-full flex items-center justify-center z-10 transition-all duration-500 ${
                  isCompleted ? "bg-gold-400 text-white shadow-gold scale-110" : "bg-white dark:bg-dark-800 text-gray-400 border-2 border-gold-400/10"
                }`}>
                ...
                <div className="flex-1 pb-4 border-b border-gold-400/5">
                   <div className="flex items-center justify-between">
                      <h3 className={`text-lg font-bold transition-colors ${isCompleted ? "text-dark-900 dark:text-cream-50" : "text-gray-400"}`}>
                        {step.label}
                      </h3>
                      {isCompleted && (
                   ...
                   {isCurrent && order.status === 'SHIPPED' && order.trackingNumber && (
                     ...
                     >
                        ...
                        <div className="flex items-center justify-between">
                           <span className="text-xl font-accent font-bold text-gold-400">{order.trackingNumber}</span>
      ...
      <div className="bg-dark-900 text-white p-8 rounded-[40px] shadow-gold-lg border border-gold-400/10 flex items-center justify-between">
         <div className="flex items-center gap-6">
            ...
            <div>
               <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">Shipping To</p>
