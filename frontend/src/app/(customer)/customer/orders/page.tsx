...
export default function MyOrdersPage() {
	...
	const confirmReceipt = useMutation({
		...
		onSuccess: () => {
			...
			toast.success(
				"Delivery confirmed! Your escrow has been released to the seller.",
			);
		},
	});
			...
			<div className="grid gap-6">
				{isLoading ? (
					...
					<div className="text-center py-32 bg-white dark:bg-dark-900 rounded-[40px] border border-gold-400/10">
						...
						<Link
							href="/products"
							className="px-10 py-4 bg-gold-400 text-white rounded-pill font-bold shadow-gold inline-flex items-center gap-2"
						>
							...
							<div className="flex-1 space-y-6">
								<div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
									<div className="space-y-1">
										<div className="flex items-center gap-3">
											<span
												className={`px-3 py-1 rounded-pill text-[10px] font-bold ${getStatusStyle(order.status)}`}
											>
												{order.status}
											</span>
										...
										<p className="text-sm text-gold-400 font-bold uppercase tracking-widest">
											{order.product?.brand || "Premium"}
										</p>
									</div>
								...
								<div className="flex items-center gap-4 p-4 bg-cream-50 dark:bg-dark-800 rounded-2xl border border-gold-400/5">
									...
									<div>
										<p className="text-xs font-bold text-gray-500">
											Seller: {order.seller?.user?.name || "Verified Merchant"}
										</p>
								...
								<div className="flex flex-wrap items-center gap-3 pt-2">
									<Link
										...
										className="px-6 py-3 bg-white dark:bg-dark-800 border-2 border-gold-400/20 text-gold-400 rounded-pill text-sm font-bold hover:bg-gold-400/5 transition-all"
									>
										View Details
									</Link>
									...
									{order.status === "SHIPPED" && (
										<Link
											...
											className="px-6 py-3 bg-gold-400/10 text-gold-400 rounded-pill text-sm font-bold flex items-center gap-2"
										>
									...
									{["PAID", "SHIPPED", "DELIVERED"].includes(order.status) && (
										<Link
											...
											className="px-6 py-3 text-red-500 font-bold text-xs hover:underline ml-auto"
										>
											Open Dispute
										</Link>
									)}
