          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
             <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                   <h3 className="text-xl font-display font-bold">Urgent: Payment Rejected</h3>
                   <p className="text-sm opacity-90 max-w-xl">
                     You have {rejectedOrders.length} order{rejectedOrders.length > 1 ? 's' : ''} with payment issues. Please re-upload your receipts to secure your items.
                   </p>
                </div>
             </div>
             <Link 
         ...
         <div className="md:col-span-2 p-6 md:p-10 bg-gradient-to-br from-emerald-500/10 to-teal-500/10 rounded-3xl md:rounded-[48px] border border-emerald-500/20 relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-8">
               <div className="space-y-2">
                  <h2 className="text-xl md:text-3xl font-display font-bold text-emerald-600 dark:text-emerald-400">Earth Impact Score</h2>
               ...
               <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-1">
                     ...
                     <p className="text-2xl md:text-4xl font-accent font-bold text-emerald-600 dark:text-emerald-400">{carbonOffset}kg <span className="text-xs md:text-sm font-medium opacity-60">CO2</span></p>
                  </div>
                  <div className="space-y-1">
                     ...
                     <p className="text-2xl md:text-4xl font-accent font-bold text-teal-600 dark:text-teal-400">{waterSaved.toLocaleString()}L <span className="text-xs md:text-sm font-medium opacity-60">H2O</span></p>
                  </div>
               </div>
            </div>
         </div>
...
function OrderCard({ order }: any) {
  const getStatusInfo = (status: string) => {
    if (order.paymentRejected) {
       return { label: 'Action Required', step: 0, desc: `Payment rejected: ${order.rejectionReason}`, color: 'text-red-500' };
    }
    switch(status) {
      case 'PAID': return { label: 'In Queue', step: 1, desc: 'Admin has verified your payment. Seller is preparing for shipment.', color: 'text-emerald-500' };
      case 'PAYMENT_SUBMITTED': return { label: 'Under Audit', step: 1, desc: 'Receipt submitted successfully. Please wait for admin to verify your payment.', color: 'text-amber-500' };
      case 'SHIPPED': return { label: 'On the Way', step: 3, desc: 'Your luxury item is moving through the courier network.', color: 'text-purple-500' };
      case 'DELIVERED': return { label: 'Arrival', step: 4, desc: 'Package delivered. Please confirm receipt to release funds.', color: 'text-blue-500' };
      default: return { label: 'Pending Audit', step: 0, desc: 'Awaiting neural payment audit.', color: 'text-amber-500' };
    }
  };
       ...
       <div className="flex flex-col md:flex-row justify-between gap-6">
          <div className="flex gap-6">
             ...
             <div className="space-y-2">
                ...
                <h4 className="text-lg md:text-2xl font-display font-bold">{order.product?.title}</h4>
          ...
          <div className="md:text-right">
             <p className="text-xl md:text-3xl font-accent font-bold text-gold-400">Rs. {order.totalPrice?.toLocaleString()}</p>
       ...
       <div className="space-y-8">
          <div className="flex items-center justify-between">
             <div className="flex items-center gap-3">
                <div className={`w-10 h-10 ${order.paymentRejected ? 'bg-red-500' : 'bg-gold-400'} text-white rounded-xl flex items-center justify-center shadow-lg`}>
                   {order.paymentRejected ? <AlertTriangle className="w-5 h-5" /> : <Truck className="w-5 h-5" />}
                </div>
                <div>
          ...
          <p className={`text-xs font-medium italic p-4 rounded-2xl border ${
            order.paymentRejected ? 'bg-red-500/5 text-red-500 border-red-500/10' : 'bg-gold-400/5 text-gray-500 border-gold-400/10'
          }`}>"{info.desc}"</p>
       </div>

       <div className="flex gap-4">
          <Link href={`/customer/orders`} className={`flex-1 py-5 rounded-3xl font-bold flex items-center justify-center gap-2 hover:scale-[1.02] transition-all ${
            order.paymentRejected ? 'bg-red-500 text-white shadow-xl shadow-red-500/20' : 'bg-dark-900 text-white'
          }`}>
