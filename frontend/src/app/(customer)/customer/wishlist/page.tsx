			...
			{isLoading ? (
				<div className="grid grid-cols-2 md:grid-cols-4 gap-8">
					{Array(4)
						...
						.map((_, i) => (
							<div
								key={i}
								className="aspect-[3/4] bg-gold-400/5 rounded-[32px] animate-pulse"
							/>
						))}
				</div>
			) : wishlist?.length === 0 ? (
				<div className="text-center py-32 bg-white dark:bg-dark-900 rounded-[40px] border border-gold-400/10 space-y-6">
					...
					<p className="text-gray-500 max-w-xs mx-auto">
						Start exploring our collection and save the styles you love!
					</p>
					<Link
						href="/products"
						className="inline-flex items-center gap-2 px-10 py-4 bg-gold-400 text-white rounded-pill font-bold shadow-gold hover:scale-105 transition-all"
					>
