...
export default function SellerPayoutsPage() {
	...
	const { data: payouts, isLoading } = useQuery({
		...
		queryFn: async () => {
			...
			return data.filter(
				(t: any) => t.type === "DEBIT" || t.status === "PENDING" || t.sellerId,
			);
		},
	});
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
				<div>
					...
					<p className="text-gray-500">
						Securely withdraw your earnings to saved accounts.
					</p>
				</div>
				<button
			...
			<div className="grid md:grid-cols-2 gap-8">
				<div className="bg-dark-900 rounded-[40px] p-10 border border-gold-400/20 relative overflow-hidden group">
					<div className="relative z-10 space-y-8">
						...
						<h2 className="text-5xl font-accent font-bold text-white">
							Rs. {balance?.pendingBalance?.toLocaleString() || 0}
						</h2>
						<div className="flex items-center gap-2 text-emerald-500 text-xs font-bold">
							<ShieldCheck className="w-4 h-4" /> Securely held in Preloved
							Vault
						</div>
					</div>
				...
				<div className="bg-white dark:bg-dark-800 rounded-[40px] p-10 border border-gold-400/10 flex flex-col justify-between">
					<div className="space-y-4">
						...
						<p className="text-gray-500 text-sm leading-relaxed">
							...
							to your balance as soon as the buyer approves the delivery and
							confirms the product is correct.{" "}
							{minPayout > 0 && (
								<>
					...
					<div className="pt-6 border-t border-gold-400/10 mt-6">
						<div className="flex items-center gap-2 text-gold-400 text-xs font-bold uppercase">
							<ShieldCheck className="w-4 h-4" /> Instant release upon buyer
							approval
						</div>
					</div>
				</div>
				...
				<div className="grid gap-4">
					{isLoading ? (
						...
						payouts.map((p: any) => (
							<div
								key={p.id}
								className="bg-white dark:bg-dark-800 p-6 rounded-3xl border border-gold-400/10 flex items-center justify-between group hover:border-gold-400/30 transition-all"
							>
								<div className="flex items-center gap-5">
									<div
										className={`w-12 h-12 rounded-xl flex items-center justify-center ${p.status === "COMPLETED" ? "bg-emerald-500/10 text-emerald-500" : "bg-amber-500/10 text-amber-500"}`}
									>
									...
									<div>
										...
										<p className="text-[10px] text-gold-400/80 font-bold">
											{p.details}
										</p>
								...
								<div className="flex flex-col items-end gap-2">
									<span
										className={`px-4 py-1.5 rounded-pill text-[10px] font-bold uppercase tracking-widest ${p.status === "COMPLETED" ? "bg-emerald-500/10 text-emerald-500" : "bg-amber-500/10 text-amber-500"}`}
									>
										{p.status}
									</span>
									{p.proofImage && (
					...
					<div className="fixed inset-0 z-[9999] flex items-end md:items-center justify-center">
						...
						>
							<div className="flex items-center justify-between mb-8">
								...
								<button
									onClick={() => setIsModalOpen(false)}
									className="p-3 bg-gold-400/10 text-gold-400 rounded-2xl hover:scale-110 transition-transform"
								>
									<X className="w-6 h-6" />
								</button>
							</div>
								...
								<form onSubmit={handleSubmit} className="space-y-8">
									<div className="space-y-4">
										...
										<input
											...
											placeholder={
												minPayout > 0
													? `Min ${minPayout.toLocaleString()} PKR`
													: "Enter Amount"
											}
									...
									<div className="space-y-4">
										...
										<div className="grid grid-cols-3 gap-4">
											{[
												...
												{ id: "EASYPAISA", icon: Banknote, label: "Easypaisa" },
											].map((m) => (
												<button
													key={m.id}
													type="button"
