...
export default function SellerProductsPage() {
	...
	const handleUpdateStock = async (
		...
		const baseStock = parseInt(currentStock?.toString() || "0", 10) || 0;
	...
	const handleEditSubmit = async (e: React.FormEvent) => {
		...
		setIsUpdating(true);
		try {
			const { id, title, description, size, category, condition, stock } =
				editingProduct;
			...
			setProducts(
				products.map((p) =>
					p.id === id
						? { ...p, title, description, size, category, condition, stock }
						: p,
				),
			);
			...
			<div className="max-w-7xl mx-auto px-2 md:px-6 lg:px-8">
				...
				<div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
					<div className="space-y-2">
						...
						<h1 className="text-4xl lg:text-5xl font-display font-bold text-dark-900 dark:text-cream-50 leading-tight">
							My <span className="italic text-gold-400">Products.</span>
						</h1>
						<p className="text-gray-500 text-sm max-w-md">
							Manage your luxury listings, track stock levels, and update piece
							details.
						</p>
					</div>
					...
					<div className="flex items-center gap-3">
						<div className="flex bg-white dark:bg-dark-900 rounded-2xl p-1 border border-gold-400/10 shadow-sm">
							<button
								...
								className={`p-2.5 rounded-xl transition-all ${viewMode === "list" ? "bg-gold-400 text-white shadow-gold" : "text-gray-400 hover:text-gold-400"}`}
							>
								<ListIcon className="w-5 h-5" />
							</button>
							<button
								onClick={() => setViewMode("grid")}
								className={`p-2.5 rounded-xl transition-all ${viewMode === "grid" ? "bg-gold-400 text-white shadow-gold" : "text-gray-400 hover:text-gold-400"}`}
							>
						...
						<Link
							...
							className="h-14 px-8 bg-gradient-to-r from-gold-400 to-gold-600 text-white rounded-2xl font-bold shadow-gold hover:scale-105 active:scale-95 transition-all flex items-center gap-3"
						>
					...
					<div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none flex-nowrap lg:flex-wrap">
						<div className="h-14 px-5 bg-white dark:bg-dark-900 border border-gold-400/10 rounded-2xl flex items-center gap-3 shadow-sm shrink-0 min-w-[140px]">
							...
							<div>
								...
								<p className="text-sm font-black text-dark-900 dark:text-cream-50">
						...
						<div className="space-y-2">
							...
							<p className="text-gray-500 text-sm max-w-xs mx-auto">
								{searchQuery
									...
									: "You haven't listed any products in the vault yet."}
							</p>
						</div>
						{!searchQuery && (
							<Link
								href="/seller/add-product"
								className="inline-flex h-14 px-10 items-center bg-gold-400 text-white rounded-2xl font-bold shadow-gold hover:scale-105 active:scale-95 transition-all"
							>
					...
					>
						{filteredProducts.map((p) => (
							...
							>
								{/* Image Container */}
								<div
									className={`relative flex-shrink-0 bg-gray-50 dark:bg-dark-800 rounded-xl overflow-hidden ${viewMode === "list" ? "w-24 aspect-[3/4] md:w-32 md:h-32" : "aspect-[4/3] w-full"}`}
								>
									{p.images?.[0] ? (
									...
									<div className="absolute top-1 left-1 flex flex-col gap-1">
										<span
											className={`px-1.5 py-0.5 rounded-full text-[6px] font-black uppercase tracking-widest backdrop-blur-md shadow-sm border ${
												p.status === "ACTIVE"
								...
								{/* Content Container */}
								<div
									className={`flex-1 min-w-0 w-full ${viewMode === "list" ? "px-2 md:px-0 md:pr-4" : "p-6"}`}
								>
									<div className="flex flex-col gap-0.5">
										<h3 className="text-xs md:text-lg font-bold text-dark-900 dark:text-cream-50 truncate leading-tight group-hover:text-gold-400 transition-colors uppercase">
											{p.title}
										</h3>
										<div className="flex items-center gap-2">
											<p className="text-[8px] md:text-[10px] font-black text-gold-400 uppercase tracking-widest">
												{p.brand}
											</p>
										...
										<div className="mt-0.5">
											<p className="text-xs md:text-lg font-accent font-bold text-gold-400 leading-none">
											...
											<div className="flex items-center bg-gray-50 dark:bg-dark-800 rounded-xl border border-gold-400/10 p-0.5">
												...
												<div className="px-1 flex flex-col items-center">
													<input
														...
														onBlur={(e) =>
															handleManualStockChange(p.id, e.target.value)
														}
														onKeyDown={(e) =>
															e.key === "Enter" &&
						...
						>
							...
							<div className="px-8 py-6 border-b border-gold-400/10 flex items-center justify-between bg-gold-400/5">
								<div className="flex items-center gap-3">
									...
									<div>
										<h2 className="text-xl font-display font-bold text-dark-900 dark:text-cream-50">
											Edit Piece
										</h2>
										<p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">
											Update vault listing
										</p>
									</div>
								</div>
								<button
									onClick={() => setIsEditModalOpen(false)}
									className="w-10 h-10 rounded-full hover:bg-gray-100 dark:hover:bg-dark-800 flex items-center justify-center text-gray-400 transition-all"
								>
									<X className="w-5 h-5" />
								</button>
							</div>
							...
							<form
								...
								className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-none"
							>
								<div className="grid md:grid-cols-2 gap-6">
									...
									<div className="space-y-2 opacity-60">
										...
										<p className="text-[9px] text-amber-500 font-bold mt-1 px-1 flex items-center gap-1">
											<AlertCircle className="w-3 h-3" /> Price cannot be
