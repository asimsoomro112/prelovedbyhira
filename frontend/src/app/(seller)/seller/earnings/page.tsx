...
export default function SellerEarningsPage() {
	...
	const downloadCSV = () => {
		...
		link.setAttribute(
			"download",
			`earnings_report_${new Date().toLocaleDateString()}.csv`,
		);
		...
		<div className="p-4 lg:p-8 space-y-8 lg:space-y-12">
			<div className="space-y-1">
				...
				<p className="text-gray-500 text-sm">
					Manage your income and withdrawal history.
				</p>
			</div>
								...
								{transLoading ? (
									Array(3)
										...
										.map((_, i) => (
											<tr
												key={i}
												className="animate-pulse h-16 bg-gold-400/5"
											/>
										))
									...
									<tr>
										<td
											colSpan={5}
											className="px-8 py-20 text-center text-gray-500 font-bold uppercase text-xs tracking-widest"
										>
									...
									filteredTransactions?.map((t: any) => (
										<tr
											key={t.id}
											className="hover:bg-gold-400/5 transition-all"
										>
											...
											<td className="px-8 py-6">
												<span
													className={`text-lg font-accent font-bold ${t.type === "CREDIT" ? "text-emerald-500" : "text-blue-500"}`}
												>
													{t.type === "CREDIT" ? "+" : "-"} Rs.{" "}
											...
											<td className="px-8 py-6">
												<span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 rounded-pill text-[10px] font-bold">
													COMPLETED
												</span>
											</td>
										</tr>
					...
					{transLoading ? (
						Array(3)
							...
							.map((_, i) => (
								<div
									key={i}
									className="h-24 bg-white dark:bg-dark-800 rounded-3xl animate-pulse"
								/>
							))
					) : filteredTransactions?.length === 0 ? (
						...
						filteredTransactions?.map((t: any) => (
							<div
								key={t.id}
								className="bg-white dark:bg-dark-800 p-5 rounded-3xl border border-gold-400/10 flex items-center justify-between"
							>
								<div className="flex items-center gap-4">
									<div
										className={`w-10 h-10 rounded-xl flex items-center justify-center ${t.type === "CREDIT" ? "bg-emerald-500/10 text-emerald-500" : "bg-blue-500/10 text-blue-500"}`}
									>
										{t.type === "CREDIT" ? (
									...
									<div>
										...
										<div className="flex items-center gap-2 mt-0.5">
											<span
												className={`text-[9px] font-bold uppercase tracking-widest ${t.type === "CREDIT" ? "text-emerald-500" : "text-blue-500"}`}
											>
								...
								<div className="text-right">
									<p
										className={`font-accent font-bold ${t.type === "CREDIT" ? "text-emerald-500" : "text-blue-500"}`}
									>
										{t.type === "CREDIT" ? "+" : "-"} Rs.{" "}
										{t.amount.toLocaleString()}
									</p>
...
function EarningCard({ label, value, icon, color, highlight }: any) {
	return (
		...
		>
			<div
				className={`w-14 h-14 ${color} bg-opacity-20 rounded-2xl flex items-center justify-center ${highlight ? "text-white" : "text-gold-400"}`}
			>
				{icon}
			</div>
			<div className="space-y-1 relative z-10">
				<p
					className={`text-[10px] font-bold uppercase tracking-[0.2em] ${highlight ? "text-gold-100/80" : "text-gray-400"}`}
				>
					{label}
				</p>
				<h3 className="text-2xl lg:text-3xl font-accent font-bold leading-none">
					{value}
				</h3>
			</div>
			{highlight && (
