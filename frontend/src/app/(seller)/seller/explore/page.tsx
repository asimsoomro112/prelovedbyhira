...
const categories = [
// ... (rest of categories)
  { name: "Shadi & Formal Suits", slug: 'SHADI-WEAR', icon: <Gem />, image: 'https://images.unsplash.com/photo-1583394838336-acd977736f90?auto=format&fit=crop&q=80&w=600', count: 320 },
  { name: "Luxury Bridal", slug: 'BRIDAL', icon: <Sparkles />, image: 'https://images.unsplash.com/photo-1594463750939-ebb28c3f7f75?auto=format&fit=crop&q=80&w=600', count: 45 },
  { name: "Kurtas & Shirts", slug: 'KURTAS', icon: <TrendingUp />, image: 'https://images.unsplash.com/photo-1591192921508-30113f89a9f2?auto=format&fit=crop&q=80&w=600', count: 540 },
  { name: "Designer Shoes", slug: 'SHOES', icon: <Zap />, image: 'https://images.unsplash.com/photo-1549298916-b41d501d3772?auto=format&fit=crop&q=80&w=600', count: 210 },
  { name: "Premium Watches", slug: 'WATCHES', icon: <Star />, image: 'https://images.unsplash.com/photo-1524592094714-0f0654e20314?auto=format&fit=crop&q=80&w=600', count: 125 },
  { name: "Bags & Accessories", slug: 'BAGS', icon: <ShoppingBag />, image: 'https://images.unsplash.com/photo-1584917865442-de89df76afd3?auto=format&fit=crop&q=80&w=600', count: 430 },
];
    ...
    <div className="min-h-screen pb-32 bg-white dark:bg-dark-950 transition-colors duration-500">
      ...
      <section className="relative pt-32 pb-20 px-6 lg:px-8 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto text-center space-y-12">
          ...
          >
            ...
            <p className="text-dark-700/60 dark:text-cream-50/60 text-lg font-medium">Browse Pakistan's most exclusive preloved fashion collections.</p>
          </motion.div>
          ...
          <div className="relative max-w-2xl mx-auto group">
             ...
             <div className="flex flex-wrap justify-center gap-3 mt-6">
                {['LV', 'Bridal', 'Kurta', 'Gucci', 'Shoes'].map(tag => (
                  <span key={tag} className="px-4 py-1.5 glass-crystal crystal-border rounded-full text-[10px] font-bold text-gold-400 uppercase tracking-widest cursor-pointer hover:bg-gold-400/10 transition-colors">
                    {tag}
                  </span>
                ))}
             </div>
      ...
      <section className="px-6 lg:px-8 py-20">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-12">
            <h2 className="text-3xl font-display font-bold text-dark-900 dark:text-cream-50">Curated <span className="text-gold-400 italic">Portals.</span></h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {categories.map((cat, i) => (
              ...
              >
                <img src={cat.image} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-[3s]" alt={cat.name} />
                ...
                <div className="absolute bottom-8 left-8 right-8 flex items-end justify-between text-white">
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] font-bold text-gold-400 uppercase tracking-[0.3em]">{cat.count}+ Items Available</p>
                  </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-end justify-between mb-16">
            <div className="space-y-4">
              ...
              <h2 className="text-4xl lg:text-6xl font-display font-bold text-dark-900 dark:text-cream-50">Latest <span className="text-gold-400 italic">Luxury.</span></h2>
            </div>
            <Link href="/products" className="text-gold-400 font-bold hover:underline flex items-center gap-2">View All Drops <ArrowRight className="w-4 h-4" /></Link>
          </div>
             ...
             {latestProducts?.map((product: any) => (
               <Link href={`/product/${product.id}`} key={product.id} className="group cursor-pointer">
                  ...
                  <div className="space-y-3 px-4">
                     ...
                     <h3 className="text-xl font-display font-bold text-dark-900 dark:text-cream-50 group-hover:text-gold-400 transition-colors line-clamp-1">{product.title || product.name}</h3>
                     <p className="text-2xl font-display font-bold text-gold-400">Rs. {(product.sellingPrice || product.price || 0).toLocaleString()}</p>
                  </div>
               </Link>
             ))}
          </div>
      ...
      <section className="px-6 lg:px-8 py-32">
        <div className="max-w-5xl mx-auto glass-ultra crystal-border rounded-[64px] p-20 text-center space-y-10 relative overflow-hidden">
          ...
          <h2 className="text-4xl lg:text-6xl font-display font-bold text-dark-900 dark:text-cream-50 relative z-10">Can't find what <br/>you're <span className="text-gold-400 italic">looking for?</span></h2>
          <p className="text-dark-700/60 dark:text-cream-50/60 max-w-lg mx-auto relative z-10 text-lg">Our AI Concierge can help you track down specific brands or sizes. We'll notify you as soon as they hit the vault.</p>
