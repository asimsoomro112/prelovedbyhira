...
export default function SellerDashboard() {
	...
	if (isLoading)
		return (
			<div className="h-screen flex items-center justify-center font-display text-gold-400">
				Initializing Vault...
			</div>
		);
		...
		<div className="max-w-screen-xl mx-auto px-4 md:px-6 py-6 lg:py-24 space-y-8 md:space-y-12 pb-32">
			...
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
				<div className="space-y-2">
					<h1 className="text-fluid-section font-display font-bold text-dark-900 dark:text-cream-50">
						Seller <span className="italic text-gold-400">Vault.</span>
					</h1>
				...
				<div className="flex items-center gap-4 w-full md:w-auto">
					<Link
						href="/products"
						className="flex-1 md:flex-none h-14 md:h-16 px-8 bg-white dark:bg-dark-800 border-2 border-gold-400/20 text-gold-500 rounded-2xl font-bold hover:bg-gold-400/5 transition-all flex items-center justify-center gap-3"
					>
						<ShoppingBag className="w-5 h-5" /> Explore Marketplace
					</Link>
					<Link
						href="/seller/add-product"
						className="flex-1 md:flex-none h-14 md:h-16 px-8 md:px-10 bg-gold-400 text-white rounded-2xl font-bold shadow-gold hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3"
					>
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
				<StatCard
					...
					trend={stats?.totalEarnings > 0 ? "+12.5%" : "Stable"}
					icon={<DollarSign />}
					primary
				/>
				<StatCard
					...
					trend={stats?.activeProducts > 0 ? "+1" : "Empty"}
					icon={<ShoppingBag />}
				/>
				<StatCard
			...
			<div className="grid lg:grid-cols-3 gap-8 md:gap-12">
				...
				<div className="lg:col-span-2 space-y-8">
					<div className="flex items-center justify-between">
						...
						<Link
							href="/seller/orders"
							className="text-xs font-bold text-gold-400 hover:underline flex items-center gap-2"
						>
					...
					<div className="space-y-4">
						{!isVerified && (
							<ActionItem
								...
								desc={
									isPending
										? "Our team is reviewing your documents. Usually takes 24-48h."
										: isRejected
											? "Review the feedback and retry verification."
											: isIdentityVerified
												? "Identity scan complete! Now please upload your selfie to finish."
												: "Please upload your ID to unlock withdrawals."
								}
								action={
									isPending
										? "Check Status"
						...
						<div className="p-8 rounded-[32px] bg-gold-400 text-white space-y-4 shadow-gold relative overflow-hidden">
							...
							<h3 className="text-xl font-bold font-display relative z-10">
								Seller Tip
							</h3>
							<p className="text-xs leading-relaxed opacity-95 relative z-10 font-medium">
								Adding a "Seller Honesty Note" increases conversion by 40% on
								preloved items.
							</p>
						</div>
					</div>
	...
	return (
		...
		>
			<div className="flex justify-between items-start">
				<div
					className={`w-12 h-12 rounded-2xl flex items-center justify-center ${primary ? "bg-gold-400 text-white shadow-gold" : "bg-gold-400/10 text-gold-400"}`}
				>
					{icon}
				</div>
				<span
					className={`text-[10px] font-bold px-2 py-1 rounded-lg ${trend.startsWith("+") ? "bg-emerald-500/10 text-emerald-500" : "bg-gold-400/10 text-gold-400"}`}
				>
					{trend}
				</span>
			</div>
	...
	return (
		<div className="p-4 md:p-6 glass-crystal crystal-border rounded-2xl md:rounded-3xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 group hover:bg-gold-400/5 transition-all">
			<div className="flex items-center gap-4 md:gap-6 w-full sm:w-auto">
				<div
					className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center font-bold text-[10px] md:text-xs shrink-0 ${urgent ? "bg-amber-500/10 text-amber-500" : "bg-gold-400/10 text-gold-400"}`}
				>
					{id}
				</div>
				<div className="space-y-0.5 md:space-y-1 min-w-0 flex-1">
					<h4 className="font-bold text-sm md:text-base text-dark-900 dark:text-cream-50 truncate">
						{item}
					</h4>
					<p className="text-[10px] md:text-xs text-gray-500">
						{time} •{" "}
						<span
							className={urgent ? "text-amber-500 font-bold" : "font-medium"}
						>
							{status}
						</span>
					</p>
			...
			<div className="flex items-center justify-between sm:justify-end gap-4 md:gap-6 w-full sm:auto pt-2 sm:pt-0 border-t sm:border-0 border-gold-400/5">
				<span className="text-base md:text-lg font-accent font-bold text-gold-400">
					{price}
				</span>
				<Link
					href="/seller/orders"
					className="w-10 h-10 rounded-xl glass-ultra crystal-border flex items-center justify-center opacity-100 sm:opacity-0 group-hover:opacity-100 transition-all active:scale-90"
	...
	return (
		<div className="p-5 md:p-6 glass-crystal crystal-border rounded-2xl md:rounded-3xl flex gap-4 md:gap-6 items-start">
			...
			<div className="space-y-3">
				<div className="space-y-1">
					<h4 className="text-sm font-bold text-dark-900 dark:text-cream-50">
						{title}
					</h4>
				...
				{link ? (
					<Link
						href={link}
						className="text-[10px] font-bold text-gold-400 uppercase tracking-widest hover:underline block"
					>
						{action}
					</Link>
				) : (
					<button className="text-[10px] font-bold text-gold-400 uppercase tracking-widest hover:underline">
						{action}
					</button>
				)}
			</div>
