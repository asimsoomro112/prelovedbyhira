...
export default function AddProductPage() {
	...
	if (vStatus !== "APPROVED" && vStatus !== "ACTIVE") {
		...
		return (
			<div className="max-w-2xl mx-auto py-24 px-6 text-center space-y-8">
				<div
					className={`w-24 h-24 rounded-full flex items-center justify-center mx-auto shadow-2xl ${isPending ? "bg-gold-400/10 text-gold-400" : "bg-red-500/10 text-red-500"}`}
				>
					{isPending ? (
				...
				<div className="space-y-4">
					<h1 className="text-4xl font-display font-bold">
						Verification{" "}
						<span className={isPending ? "text-gold-400" : "text-red-500"}>
							{isPending ? "Under Review." : "Required."}
						</span>
					</h1>
				...
				<div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
					<button
						onClick={() =>
							router.push(
								isPending ? "/seller/dashboard" : "/seller/verification",
							)
						}
	...
	const onSubmit = async (values: ProductFormValues) => {
		...
		try {
			...
			toast.success(
				"Product published successfully! It will be live after admin review.",
			);
	...
	const onInvalid = (errors: any) => {
		...
		toast.error(
			firstError?.message || "Please check all fields and try again.",
		);
	};
		...
		<div className="max-w-4xl mx-auto px-4 py-8 md:p-12 space-y-8 md:space-y-12">
			...
			<div className="space-y-4 md:space-y-6">
				<div className="flex items-center justify-between">
					<h1 className="text-fluid-section font-display font-bold">
						List Item
					</h1>
						...
						>
							<div className="space-y-4">
								...
								<p className="text-gray-500 text-sm">
									Add up to 8 high-quality photos. Show all angles and any
									defects.
								</p>
							</div>
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								...
								{previews.map((preview, i) => (
									<div
										key={i}
										className="relative aspect-square rounded-3xl overflow-hidden border border-gold-400/10 group bg-white dark:bg-dark-900"
									>
										<Image
											src={preview}
											alt="Preview"
										...
										/>
										{i === 0 && (
											<div className="absolute top-2 left-2 bg-gold-400 text-white text-[8px] font-bold px-2 py-1 rounded-full shadow-gold z-10">
												COVER
											</div>
										)}
										<button
							...
							<div className="space-y-4 pt-4">
								...
								<p className="text-gray-500 text-sm">
									Upload a short video (max 50MB) to show the luxury detail in
									motion.
								</p>
						...
						>
							...
							<div className="grid md:grid-cols-2 gap-8">
								<div className="md:col-span-2 space-y-2">
									...
									<input
										{...register("title")}
										placeholder="e.g. Vintage Zara Silk Dress"
										className="add-input"
									/>
									{errors.title && (
								...
								<div className="md:col-span-2 space-y-4">
									...
									<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
										{["EXCELLENT", "GOOD", "FAIR", "POOR"].map((cond) => (
											...
											>
												<Star
													className={`w-5 h-5 ${watch("condition") === cond ? "fill-gold-400 text-gold-400" : "text-gray-300"}`}
												/>
								...
								<div className="md:col-span-2 space-y-2">
									...
									<textarea
										...
										placeholder="Describe the fit, fabric, and any details..."
									/>
									{errors.description && (
										<p className="text-xs text-red-500">
								...
								<div className="md:col-span-2 space-y-2">
									...
									<textarea
										{...register("defects")}
										rows={2}
										className="add-input resize-none border-amber-500/20 bg-amber-500/5"
										placeholder="Mention any marks, stains, or missing buttons..."
									/>
								</div>
								...
								<div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
									<div className="flex items-center justify-between p-4 bg-white dark:bg-dark-900 rounded-2xl border border-gold-400/10">
										<div className="flex flex-col gap-1">
											...
											<span className="text-[10px] text-gray-400">
												Box, bag, or original tags included?
											</span>
										</div>
										<button
											type="button"
											onClick={() =>
												setValue("originalPacking", !watch("originalPacking"))
											}
									...
									<div className="flex items-center justify-between p-4 bg-white dark:bg-dark-900 rounded-2xl border border-gold-400/10">
										<div className="flex flex-col gap-1">
											...
											<span className="text-[10px] text-gray-400">
												Original receipt or digital proof?
											</span>
										</div>
										<button
											type="button"
											onClick={() =>
												setValue("invoiceAvailable", !watch("invoiceAvailable"))
											}
							...
							<div className="flex flex-col-reverse sm:flex-row justify-between gap-3 pt-4">
								<button
									type="button"
									onClick={() => setStep(1)}
									className="w-full sm:w-auto px-8 py-4 text-gold-400 font-bold flex items-center justify-center gap-2 min-h-[48px] active:bg-gold-400/5 rounded-2xl transition-all"
								>
								...
								<button
									type="button"
									onClick={() => setStep(3)}
									className="w-full sm:w-auto px-8 py-4 bg-gold-400 text-white rounded-2xl font-bold shadow-gold flex items-center justify-center gap-2 min-h-[52px] active:scale-95 transition-all text-sm"
								>
									Next Step <ArrowRight className="w-5 h-5" />
								</button>
							</div>
						...
						>
							<div className="text-center space-y-4">
								...
								<p className="text-gray-500">
									Almost there! Review your item before it goes live.
								</p>
							</div>
								...
								<div className="space-y-6">
									<div className="space-y-2">
										...
										<div className="flex items-center gap-4 pt-2">
											<p className="text-3xl font-accent font-bold text-gold-400">
												Rs. {watch("sellingPrice")}
											</p>
											<p className="text-gray-400 line-through">
												Rs. {watch("originalPrice")}
											</p>
										</div>
									</div>
									...
									<div className="p-6 bg-gold-400/5 border border-gold-400/20 rounded-3xl">
										...
										<p className="text-xs text-gray-500 leading-relaxed">
											ReVault takes a 20% platform fee. You will receive{" "}
											<span className="font-bold text-gold-400">
												Rs. {Math.round(watch("sellingPrice") * 0.8)}
											</span>{" "}
											from this sale.
										</p>
									</div>
