...
export default function SellerVerificationPage() {
		...
		try {
			...
			toast.error(
				e.response?.data?.message ||
					"AI Verification failed. Try with a clearer photo.",
			);
		} finally {
	...
	const onSubmit = async (values: VerificationValues) => {
		...
		try {
			...
			await api.post("/seller/submit-selfie", fd, {
				headers: { "Content-Type": "multipart/form-data" },
			});
		...
		<div className="min-h-screen w-full max-w-5xl mx-auto px-4 py-10 lg:py-16 space-y-14">
			...
			>
				...
				<p className="text-sm text-gray-500 max-w-lg mx-auto leading-relaxed">
					Complete these steps to unlock your luxury boutique and start selling
					premium fashion across Pakistan.
				</p>
			</motion.div>
				...
				>
					...
					<div className="relative z-10 flex justify-between">
								...
								<div key={n} className="flex flex-col items-center gap-3">
									<motion.div
										animate={
												...
												? { backgroundColor: "rgba(212,175,55,1)", scale: 1 }
												: active
													? {
                      ...
                      ${
													...
													? "border-gold-400 text-white shadow-gold"
													: active
                    ...
                    `}
									...
									<span
										className={`text-[9px] font-black uppercase tracking-[0.22em] transition-colors ${active || done ? "text-gold-400" : "text-gray-500"}`}
									>
										{label}
									</span>
								</div>
			...
			<div className="relative">
				...
				<div
					...
					style={{
						background:
							"radial-gradient(ellipse 70% 60% at 50% 20%, rgba(212,175,55,0.18) 0%, transparent 70%)",
						filter: "blur(24px)",
					}}
				/>
				...
				<div className="relative glass-ultra crystal-border rounded-[48px] overflow-hidden shadow-gold-3d">
					...
					<div
						aria-hidden
						className="absolute top-0 right-0 w-80 h-80 bg-gold-400/4 blur-[80px] rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none"
					/>
									...
									>
										...
										<div className="flex items-start gap-4 p-5 rounded-2xl bg-gold-400/5 border border-gold-400/15">
											...
											<p className="text-xs text-gray-500 leading-relaxed">
												Aapki personal information end-to-end encrypted hai aur
												sirf verification ke liye use hogi. Hum kabhi aapka data
												third parties ke saath share nahi karte.
											</p>
										</div>
									...
									>
										...
										<div className="flex items-center gap-3 p-4 rounded-2xl border border-gold-400/20 bg-gold-400/5">
											...
											<p className="text-xs text-gray-500">
												...
												will extract your CNIC number and cross-match your
												entered name. Images are deleted immediately after
												processing.
											</p>
										</div>
											...
											{isAIProcessing && (
												...
												>
													...
													<div className="text-center space-y-2">
														...
														<p className="text-xs text-gold-400/70 uppercase tracking-[0.25em]">
															ReVault AI at work…
														</p>
													</div>
								...
								{step === 3 && (
									...
									>
										...
										<div className="grid grid-cols-3 gap-3">
											{["Good lighting", "Face centered", "No glasses"].map(
												(tip) => (
													<div
														key={tip}
														className="p-3 rounded-2xl bg-gold-400/5 border border-gold-400/10 text-center"
													>
														<p className="text-[10px] font-bold text-gold-400 uppercase tracking-wider">
															{tip}
														</p>
													</div>
												),
								...
								{step === 4 && (
									...
									>
										...
										>
											...
											<motion.div variants={fieldItem}>
												<p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.25em] mb-3">
													Payment Method
												</p>
												<div className="grid grid-cols-3 gap-3">
													{[
										...
										<div className="flex gap-4">
											...
											<PrimaryButton type="submit" disabled={isSubmitting}>
												{isSubmitting ? (
													...
													<>
														<BadgeCheck className="w-5 h-5" /> Complete
														Application
													</>
												)}
											</PrimaryButton>
											...
											return (
												<>
													<div className="relative w-40 h-40">
														...
														{/* Center icon */}
														<div
															className={`absolute inset-6 rounded-full flex items-center justify-center ${approved ? "bg-emerald-500/10" : "bg-gold-400/10"}`}
														>
															{approved ? (
													...
													<div className="space-y-3 max-w-md">
														<h2
															className={`text-4xl font-display font-black ${approved ? "text-emerald-500" : "text-gold-400"}`}
														>
													...
													<p className="text-[9px] text-gray-400 uppercase tracking-[0.25em] font-bold">
														Email aur dashboard notification ke zariye update
														milegi
													</p>
												</>
											);
									...
									>
										...
										<div className="space-y-3">
											...
											<p className="text-sm text-gray-500 italic">
												Admin ne aapke submission pe feedback diya hai.
											</p>
										</div>
										...
										<div className="w-full max-w-lg p-6 rounded-3xl bg-red-500/5 border border-red-500/20 text-left space-y-3">
											<div className="flex items-center gap-2">
												...
												<p className="text-[10px] font-black text-red-500 uppercase tracking-[0.25em]">
													Reason for Rejection
												</p>
											</div>
											<p className="text-sm text-dark-900 dark:text-cream-50 leading-relaxed font-medium">
												{rejectionReason ||
													"Please ensure your documents are clear and valid according to our platform standards."}
											</p>
										</div>
...
function StepHeading({
	...
	return (
		<div className="space-y-3">
			<p className="text-[10px] font-black text-gold-400 uppercase tracking-[0.28em]">
				{tag}
			</p>
			<h2 className="text-3xl font-display font-black text-dark-900 dark:text-cream-50 leading-tight">
				{title}
			</h2>
