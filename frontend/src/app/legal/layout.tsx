...
import { Shield, Lock, Scale, FileText, CheckCircle2, Gavel } from "lucide-react";
    ...
    <div className="min-h-screen bg-mesh dark:bg-black py-10 lg:py-16 print:py-0 print:bg-white">
      ...
      <div className="max-w-4xl mx-auto px-6 print:px-0 print:max-w-none">
        ...
        >
          ...
          <div className="p-6 md:p-12 border-b border-gold-400/10 bg-gradient-to-br from-gold-400/5 to-transparent text-center relative overflow-hidden">
             ...
             <h1 className="text-4xl md:text-6xl font-display font-bold text-dark-900 dark:text-cream-50">
                {pathname === '/legal/terms' ? 'Terms of Service' : 
                 pathname === '/legal/privacy' ? 'Privacy Policy' : 
                 pathname === '/legal/escrow' ? 'Escrow Policy' : 'Legal Vault'}
          ...
          <div className="p-8 bg-gold-400/5 border-t border-gold-400/10 flex flex-col md:flex-row items-center justify-between gap-6">
             <div className="flex items-center gap-4">
                ...
                <div>
                   <p className="text-sm font-bold text-dark-900 dark:text-cream-50">Encrypted Legal Data</p>
                   <p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest">End-to-End Secure</p>
                </div>
             </div>
             <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
           <LegalNavCard icon={<FileText />} label="Terms" href="/legal/terms" active={pathname === '/legal/terms'} />
           <LegalNavCard icon={<Lock />} label="Privacy" href="/legal/privacy" active={pathname === '/legal/privacy'} />
           <LegalNavCard icon={<Gavel />} label="Escrow Policy" href="/legal/escrow" active={pathname === '/legal/escrow'} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
       <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
         active ? "bg-gold-400 text-white scale-110 shadow-gold" : "bg-gold-400/10 text-gold-400 group-hover:scale-110"
       }`}>
          {icon}
       </div>
       <span className={`font-bold text-sm ${active ? "text-gold-400" : ""}`}>{label}</span>
    </a>
  );
}
