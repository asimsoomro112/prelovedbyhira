...
import { MessageCircle, Send, X, ShieldCheck, Loader2, User } from "lucide-react";
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="p-6 bg-gradient-to-r from-gold-400 to-gold-600 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                   ...
                   <p className="text-[10px] opacity-80 uppercase tracking-widest font-black">24/7 Priority Support</p>
                </div>
              </div>
              <button onClick={closeChat} className="hover:rotate-90 transition-transform">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            >
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-50 px-10">
                   ...
                   <p className="text-xs font-bold leading-relaxed">Welcome to the ReVault Vault Portal. How can we assist you today?</p>
                </div>
              ) : (
                messages.map((msg) => (
                  <div 
                  ...
                  >
                    <div className={`max-w-[80%] p-4 rounded-2xl text-sm ${
                      ...
                      {msg.text}
                      <p className={`text-[8px] mt-1 ${msg.senderId === user?.id ? 'text-white/60' : 'text-gray-400'}`}>
            ...
            {/* Input */}
            <form onSubmit={handleSend} className="p-4 border-t border-gold-400/10 bg-white dark:bg-dark-950">
