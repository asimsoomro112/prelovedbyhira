		...
		>
			...
			<Link href={`/product/${product.id}`} className="block">
				<div className="relative aspect-[3/4] overflow-hidden bg-gold-400/5">
					...
					<div
						className={`absolute top-2 left-2 px-2.5 py-1 rounded-full text-[10px] font-bold text-white shadow-sm flex items-center gap-1 ${conditionColors[product.condition] || "bg-gold-400"}`}
					>
					...
					>
						<Heart
							className={`w-5 h-5 transition-colors ${wishlisted ? "fill-gold-400 text-gold-400" : "text-white"}`}
						/>
					</button>
					...
					{product.status === "SOLD" && (
						<div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] flex items-center justify-center">
							<span className="text-white text-2xl font-display italic font-bold -rotate-12 border-4 border-white px-5 py-1.5">
								SOLD
							</span>
						</div>
					)}
			...
			<div className="p-3 space-y-2">
				...
				<div className="flex items-end justify-between gap-2 pt-1">
					<div className="space-y-0.5">
						<p className="text-gold-400 font-accent font-bold text-base leading-none">
