...
export default function NotificationDropdown() {
	...
	return (
		<div className="relative">
			...
			>
				<Bell
					className={`w-5 h-5 transition-all ${isOpen ? "text-gold-400" : "text-gray-400 group-hover:text-gold-400"}`}
				/>
				{unreadCount > 0 && (
				...
				{isOpen && (
					<>
						...
						>
								...
								{notifications.length === 0 ? (
									...
									<div className="divide-y divide-gold-400/5">
										{notifications.map((n) => (
											...
											>
												<div
													className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${getColor(n.type)}`}
												>
													{getIcon(n.type)}
												</div>
												<div className="flex-1 min-w-0 space-y-1">
													<div className="flex items-center justify-between gap-2">
														<p
															className={`text-sm font-bold truncate ${!n.isRead ? "text-dark-900 dark:text-white" : "text-gray-500"}`}
														>
															{n.title}
														</p>
							...
							{/* ✅ Footer action */}
							<div
								className="p-4 bg-cream-50 dark:bg-dark-800/50 border-t border-gold-400/10 shrink-0"
								style={{
