...
export function CustomerHome({ user }: { user: any }) {
	...
	const _isPending =
		vStatus?.status === "PENDING" || vStatus?.status === "SELFIE_UPLOADED";
					...
					<div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
						<div>
							...
							<p className="mt-6 text-xl text-dark-700/60 dark:text-cream-50/60 max-w-2xl">
								Our AI has analyzed the latest luxury drops. Here is what
								matches your style profile today.
							</p>
						</div>
					...
					<div className="md:col-span-2 h-[400px] glass-ultra crystal-border rounded-[48px] p-10 flex flex-col justify-between group overflow-hidden relative shadow-gold-3d">
						<div className="relative z-10">
							...
							<h3 className="text-3xl font-display font-bold">
								New arrivals in <br />
								<span className="text-gold-400 italic">Preloved Luxury</span>
							</h3>
						</div>
						<div className="relative z-10 flex items-center justify-between">
							<Link
								href="/products"
								className="w-14 h-14 rounded-2xl bg-gold-400 text-white flex items-center justify-center hover:scale-110 transition-transform shadow-gold"
							>
					...
					<div className="h-[400px] glass-ultra crystal-border rounded-[48px] p-8 flex flex-col justify-between shadow-soft">
						...
						<div className="space-y-4">
							...
							<p className="text-xs text-gray-500 leading-relaxed">
								Stay updated on price drops for your curated favorites.
							</p>
							<Link
								href="/customer/wishlist"
								className="text-xs font-bold text-gold-400 uppercase tracking-widest hover:gap-3 transition-all flex items-center gap-2"
							>
					...
					<div className="flex items-end justify-between mb-12">
						<div className="space-y-2">
							<h2 className="text-4xl font-display font-bold">
								Live <span className="text-gold-400 italic">Marketplace.</span>
							</h2>
							<p className="text-sm text-gray-500">
								Real items currently available in the vault.
							</p>
						</div>
						<Link
							href="/products"
			...
			<section className="px-6 lg:px-8">
				<div className="max-w-7xl mx-auto">
					<div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
						<div className="space-y-4">
							...
							<h1 className="text-5xl lg:text-7xl font-display font-bold text-dark-900 dark:text-cream-50">
								Boutique <span className="text-gold-400 italic">Overview.</span>
							</h1>
							<div className="flex flex-wrap gap-4 mt-6">
								...
								<Link
									href="/seller/explore"
									className="px-8 py-4 bg-dark-900 text-white dark:bg-cream-50 dark:text-dark-900 rounded-2xl font-bold shadow-lg flex items-center gap-2 hover:scale-105 active:scale-95 transition-all"
								>
									<ShoppingBag className="w-5 h-5" /> Explore Market
								</Link>
								<Link
									href="/seller/add-product"
									className="px-8 py-4 bg-gold-400 text-white rounded-2xl font-bold shadow-gold flex items-center gap-2 hover:scale-105 active:scale-95 transition-all"
								>
									<Plus className="w-5 h-5" /> List New Item
								</Link>
								<Link
									href="/seller/dashboard"
									className="px-8 py-4 glass-ultra crystal-border rounded-2xl font-bold flex items-center gap-2 hover:bg-gold-400/5 transition-all"
								>
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
						...
						<PulseCard
							label="Shop Rating"
							value={shopHealth.rating || "5.0"}
							icon={<Star />}
							trend="Merchant Standing"
						/>
					</div>
				</div>
			</section>
					...
					<div className="md:col-span-2 h-[350px] bg-dark-900 rounded-[48px] p-10 flex flex-col justify-between text-white relative overflow-hidden shadow-2xl">
						...
						<div className="relative z-10 flex items-center gap-4">
							...
							<div>
								...
								<p className="text-[10px] text-gold-200/80 uppercase tracking-[0.2em] font-bold">
						...
						<div className="relative z-10">
							<p className="text-lg text-gray-300 max-w-lg mb-8">
								Your store is performing at{" "}
								<span className="text-gold-400 font-bold">
									{shopHealth.responseRate || "100%"} response rate
								</span>
								. High responsiveness increases your chances of a featured spot
								in the marketplace.
							</p>
							<Link
								href="/seller/dashboard"
								className="px-6 py-3 bg-white/10 hover:bg-white/20 border border-white/10 rounded-xl text-xs font-bold transition-all inline-block"
							>
								View Full Analytics
							</Link>
						</div>
						...
						<div className="space-y-4">
							...
							<p className="text-sm text-gray-500">
								...
								<b className="capitalize">
									{vStatus?.status?.replace("_", " ") || "Checking..."}
								</b>
							</p>
							<Link
								href="/seller/verification"
								className="w-12 h-12 rounded-full border border-gold-400/20 flex items-center justify-center text-gold-400 hover:bg-gold-400 hover:text-white transition-all"
							>
			...
			<section className="px-6 lg:px-8 pb-10">
				<div className="max-w-7xl mx-auto glass-ultra crystal-border rounded-[48px] p-12 shadow-soft">
					<div className="flex items-center justify-between mb-10">
						<h3 className="text-3xl font-display font-bold text-dark-900 dark:text-cream-50">
							Action <span className="text-gold-400 italic">Center.</span>
						</h3>
					</div>
				...
				<div className="max-w-7xl mx-auto">
					<div className="flex items-end justify-between mb-12 border-t border-gold-400/10 pt-16">
						<div className="space-y-4">
							...
							<h2 className="text-4xl font-display font-bold text-dark-900 dark:text-cream-50">
								Explore the <span className="text-gold-400 italic">Vault.</span>
							</h2>
							<p className="text-sm text-gray-500 max-w-md">
								See what other elite sellers are listing and find your next
								premium piece.
							</p>
						</div>
						<Link
							href="/products"
							className="group flex items-center gap-3 text-gold-400 font-bold hover:gap-5 transition-all"
						>
...
function MarketplaceGrid() {
	...
	if (!products)
		return (
			<div className="col-span-4 h-40 animate-pulse bg-gray-100 rounded-3xl" />
		);
...
function PulseCard({
	...
	icon,
	trend,
	color = "text-dark-900 dark:text-cream-50",
}: any) {
	return (
		<div className="glass-ultra crystal-border p-5 md:p-8 rounded-[40px] space-y-4 shadow-soft hover:shadow-gold-3d transition-all">
			...
			<div>
				...
				<p className="text-[10px] font-bold text-gold-400/60 mt-2 italic">
					{trend}
				</p>
			</div>
		</div>
	...
	return (
		<div className="flex flex-col md:flex-row md:items-center justify-between p-6 glass-crystal crystal-border rounded-3xl gap-6">
			<div className="flex items-center gap-6">
				...
				<div className="space-y-1">
					<h4 className="font-bold text-dark-900 dark:text-cream-50">
						{title}
					</h4>
			...
			{href ? (
				<Link
					href={href}
					className="px-6 py-3 border border-gold-400/20 rounded-xl text-xs font-bold text-gold-400 hover:bg-gold-400 hover:text-white transition-all text-center"
				>
					{action}
				</Link>
			) : (
