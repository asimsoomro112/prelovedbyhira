					...
					<div className="flex flex-col lg:flex-row items-center justify-between gap-10">
						<div className="max-w-md text-center lg:text-left">
							<h3 className="text-3xl md:text-4xl font-display font-bold leading-tight">
								Stay in the <span className="text-gold-400 italic">Vault.</span>
							</h3>
							<p className="text-gray-500 mt-4 text-sm tracking-wide">
								Be the first to secure rare preloved treasures and exclusive
								luxury insights.
							</p>
						</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-16 lg:gap-10">
					...
					<div className="lg:col-span-2 space-y-8">
						...
						<p className="text-gray-500 text-sm leading-relaxed max-w-sm">
							Pakistan's first and most secure luxury preloved marketplace. We
							leverage AI-verified authentication and secure escrow protection
							to redefine high-end fashion trade.
						</p>
					...
					{Object.entries(footerLinks).map(([title, links]) => (
						<div key={title} className="space-y-6">
							<h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-gold-400/80">
								{title}
							</h4>
							<ul className="space-y-4">
								{links.map((link) => (
									<li key={link.label}>
										<Link
											href={link.href}
											className="text-sm text-gray-400 hover:text-gold-400 transition-all flex items-center gap-2 group"
										>
