...
import {
  Home, Search, Heart, User, ShoppingBag, Sparkles,
  ChevronRight, Gem, TrendingUp, Star, Zap, X,
  LayoutDashboard, Plus, ArrowUpRight,
} from "lucide-react";
...
const navVariants = {
  ...
  visible: {
    y: 0,
    opacity: 1,
    transition: { type: "spring", stiffness: 260, damping: 28, delay: 0.15 } as const,
  },
} as const;
...
const sheetListVariants = {
  hidden: {},
  visible: { transition: { staggerChildren: 0.06, delayChildren: 0.2 } as const },
} as const;
...
const CATEGORIES = [
  { label: "Shadi & Formal", sub: "Wedding & Occasion Wear", href: "/products?category=SHADI-WEAR", icon: <Gem className="w-5 h-5" /> },
  { label: "Luxury Bridal", sub: "Curated Bridal Collection", href: "/products?category=BRIDAL", icon: <Sparkles className="w-5 h-5" /> },
  { label: "Kurtas & Shirts", sub: "Everyday & Formal Cuts", href: "/products?category=KURTAS", icon: <TrendingUp className="w-5 h-5" /> },
  { label: "Premium Watches", sub: "Swiss & Designer Pieces", href: "/products?category=WATCHES", icon: <Star className="w-5 h-5" /> },
  { label: "Designer Shoes", sub: "Luxury Footwear", href: "/products?category=SHOES", icon: <Zap className="w-5 h-5" /> },
  { label: "Bags & Jewelry", sub: "Statement Accessories", href: "/products?category=BAGS", icon: <ShoppingBag className="w-5 h-5" /> },
];
...
export default function BottomNavbar() {
  ...
  const tabs: NavTab[] = isSeller
    ? [
      ...
      { id: "explore", label: "Explore", Icon: Sparkles, onClick: () => setIsMenuOpen(true) },
    ...
    : [
      ...
      { id: "cart", label: "Cart", Icon: ShoppingBag, href: "/cart", badge: items.length },
      ...
      >
        ...
        <div className="relative glass-ultra crystal-border rounded-[32px] h-20 flex items-center justify-around px-2 shadow-gold-3d overflow-visible pointer-events-auto">
          ...
          {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <motion.div
                    ...
                    style={{
                      background: "linear-gradient(135deg, #D4AF37 0%, #B8960C 60%, #A07800 100%)",
                    }}
                  >
                    {/* FAB inner glow ring */}
                    <div
            ...
            const inner = (
              <div className="relative flex flex-col items-center justify-center gap-1">
                ...
                <span className="relative">
                  <tab.Icon
                    className={`w-5 h-5 transition-all duration-200 ${isActive ? "text-gold-400 scale-110" : "text-gray-500"
                      }`}
                  />
                ...
                <span
                  className={`text-[9px] font-bold uppercase tracking-wider transition-colors duration-200 ${isActive ? "text-gold-400" : "text-gray-500"
                    }`}
                >
                  {tab.label}
                </span>
            ...
            return (
              <motion.div key={tab.id} whileTap={{ scale: 0.88 }} className="flex-1 flex items-center justify-center h-full">
                <Link href={tab.href!} className="flex items-center justify-center w-full h-full" aria-label={tab.label}>
                  {inner}
                </Link>
              </motion.div>
            );
