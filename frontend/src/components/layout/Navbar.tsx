			...
			<div className="max-w-screen-xl mx-auto px-4 lg:px-8">
				<div className="flex h-20 items-center justify-between gap-8">
					...
					<div className="flex items-center gap-12">
						<Link
							href="/"
							className="font-display italic text-2xl shrink-0 group"
						>
							Re
							<span className="font-bold not-italic text-gold-400 group-hover:tracking-wider transition-all duration-500">
								Vault
							</span>
						</Link>
						...
						<div className="hidden lg:flex items-center gap-8">
							<Link
								href="/products"
								className={`text-[10px] font-bold uppercase tracking-[0.2em] hover:text-gold-400 transition-colors ${pathname === "/products" ? "text-gold-400" : "text-gray-500"}`}
							>
								Shop
							</Link>
							<Link
								href="/products?category=Bridal"
								className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 hover:text-gold-400 transition-colors"
							>
								Bridal
							</Link>
							<Link
					...
					<div className="hidden md:flex flex-1 max-w-xl relative group">
						<div
							className={`absolute inset-0 bg-gold-400/5 rounded-full transition-all duration-500 ${isSearchFocused ? "opacity-100 scale-105" : "opacity-0"}`}
						/>
							...
							{isSearchFocused && searchQuery.length > 2 && (
								...
								>
									...
									<div className="space-y-2">
										{["Silk Dresses", "Vintage Jewelry", "Bridal Wear"].map(
											(s) => (
												<button
													key={s}
													className="w-full text-left p-3 hover:bg-gold-400/5 rounded-xl text-sm font-medium transition-colors flex items-center justify-between"
												>
						...
						{user ? (
							<>
								...
								<Link
									...
									className="hidden lg:block p-2.5 hover:bg-gold-400/10 rounded-full text-gray-600 dark:text-gray-300"
								>
								...
								<Link
									href="/cart"
									className="relative p-2.5 hover:bg-gold-400/10 rounded-full text-gray-600 dark:text-gray-300 transition-all group"
								>
								...
								<div className="relative h-fit user-menu-container">
									...
									>
										...
										<span className="hidden lg:block text-sm font-bold text-gray-700 dark:text-cream-50">
											{user.name.split(" ")[0]}
										</span>
									</button>
												...
												<div className="bg-white dark:bg-dark-900 rounded-[32px] shadow-card border border-gold-400/10 p-3">
													...
													{isSeller && (
														<Link
															...
															className="flex items-center gap-3 p-4 rounded-2xl hover:bg-gold-400/5 text-sm font-bold transition-all"
														>
													...
													<Link
														...
														className="flex items-center gap-3 p-4 rounded-2xl hover:bg-gold-400/5 text-sm font-bold transition-all"
													>
														<User className="w-5 h-5 text-gold-400" /> My
														Profile
													</Link>
													<Link
							...
							<div className="flex items-center gap-3">
								<Link
									href="/login"
									className="hidden lg:block text-sm font-bold text-gray-500 hover:text-gold-400 transition-colors"
								>
									Login
								</Link>
								<Link
									href="/register"
									className="px-6 py-2.5 bg-gold-400 text-white rounded-pill text-sm font-bold shadow-gold hover:scale-105 transition-all"
								>
									Join Now
								</Link>
							</div>
						...
						>
							...
							<div className="flex items-center justify-between mb-12">
								<Link href="/" className="font-display italic text-2xl">
									Re<span className="text-gold-400">Vault</span>
								</Link>
								<button
									onClick={() => setIsMobileMenuOpen(false)}
									className="p-3 bg-gold-400/10 text-gold-400 rounded-2xl"
								>
									<X className="w-6 h-6" />
								</button>
							</div>
							...
							<div className="space-y-8">
								...
								<div className="relative">
									...
									<input
										placeholder="Search..."
										className="w-full bg-cream-50 dark:bg-dark-800 border-2 border-gold-400/10 rounded-2xl pl-12 pr-4 py-4 text-sm font-bold"
									/>
								</div>
...
function MobileNavLink({ href, label }: { href: string; label: string }) {
	return (
		<Link
			href={href}
			className="flex items-center justify-between p-5 rounded-2xl hover:bg-gold-400/5 transition-all group"
		>
			<span className="font-display font-bold text-xl text-gray-700 dark:text-cream-50 group-hover:text-gold-400">
				{label}
			</span>
