...
import {
  Menu, X, Bell, User, ShoppingBag, Home, Heart,
  Grid, Search, Plus, Sparkles, ChevronRight,
  LayoutDashboard, LogOut, Sun, Moon, Gem, TrendingUp,
  Star, Zap, Package, Settings,
} from "lucide-react";
...
const listContainerVariants = {
  hidden: {},
  visible: { transition: { staggerChildren: 0.055, delayChildren: 0.18 } as const },
} as const;
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            >
              {/* ── Profile Header ─────────────────────────── */}
              <div className="relative flex-shrink-0 p-6 overflow-hidden"
                style={{
                  background:
              ...
              <div className="flex-1 overflow-y-auto py-4 px-3 space-y-5">
                ...
                >
                  ...
                  <div className="space-y-0.5 mt-2">
                    <AnimLink variants={listItemVariants} icon={<Home className="w-5 h-5" />} label="Home" href="/" pathname={pathname} />
                    <AnimLink variants={listItemVariants} icon={<Grid className="w-5 h-5" />} label="Marketplace" href="/products" pathname={pathname} />
                    <AnimLink variants={listItemVariants} icon={<Heart className="w-5 h-5" />} label="Wishlist" href="/customer/wishlist" pathname={pathname} />
                    <AnimLink
                      ...
                      onClick={(e: React.MouseEvent) => {
                        if (!user) {
                          ...
                          toast.info("Sign in Required", {
                            description: "Please connect your account to view notifications.",
                ...
                >
                  ...
                  <div className="space-y-0.5 mt-2">
                    <AnimLink variants={listItemVariants} icon={<LayoutDashboard className="w-5 h-5" />} label="Dashboard" href={dashboardHref} pathname={pathname} />
                    <AnimLink variants={listItemVariants} icon={<Package className="w-5 h-5" />} label="My Orders" href={user?.role === "SELLER" ? "/seller/orders" : "/customer/orders"} pathname={pathname} />
                    <AnimLink variants={listItemVariants} icon={<Settings className="w-5 h-5" />} label="Account Settings" href={user?.role === "SELLER" ? "/seller/profile" : "/customer/profile"} pathname={pathname} />
...
function AnimLink({
  icon, label, href, pathname, highlight, onClick, variants,
}: {
  ...
  return (
    <motion.div variants={variants}>
      ...
      >
        ...
        <ChevronRight
          className={`w-4 h-4 transition-all ${isActive || highlight
